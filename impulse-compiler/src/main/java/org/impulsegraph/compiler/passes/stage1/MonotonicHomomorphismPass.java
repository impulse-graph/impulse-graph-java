...
/**
 * Stage 1 Pass: Monotonic Homomorphism Pushdown. Exploit join-semilattice
 * homomorphisms to commute monotonic functions across aggregations: max(f(X))
 * -> f(max(X)) (when f is strictly monotonic increasing) max(g(X)) -> g(min(X))
 * (when g is strictly monotonic decreasing)
 */
		...
		if (result.kind() == CelAstNode.Kind.FUNCTION_CALL) {
			...
			if (("max".equals(func) || "min".equals(func)) && children.size() == 1) {
				...
				if (arg.kind() == CelAstNode.Kind.FUNCTION_CALL && arg.signature() != null) {
					if (arg.signature().commutesWithMax()
							|| arg.signature().monotonicity() == Monotonicity.MONO_STRICT_INC) {
