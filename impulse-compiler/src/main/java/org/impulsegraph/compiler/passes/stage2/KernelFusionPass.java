 ...
 * <p>
 * Rewrites two consecutive forward CSR walks {@code (csr-walk rel1)} and
 * {@code (csr-walk rel2)} into a single fused 2-hop traversal
 * {@code (csr-walk-2hop rel1 rel2)} when the intermediate relation multiplicity
 * is below {@link OptimizerConfig#FUSED_2HOP_MAX_MULTIPLICITY_THRESHOLD} (1.5).
 * </p>
 * <p>
 * Governed by {@link OptimizerConfig#ENABLE_EXPERIMENTAL_2HOP_FUSION} /
 * {@code CompilerOptions#enableExperimental2HopFusion()}.
 * </p>
 */
public final class KernelFusionPass implements CompilerPass {
	...
	public ImpScmNode transform(ImpScmNode ast, CompilerContext context) {
		...
		if (ast instanceof ScmProgram prog) {
			...
			while (i < originalSteps.size()) {
				...
				if (i + 1 < originalSteps.size() && current instanceof ScmWalk w1
						&& originalSteps.get(i + 1) instanceof ScmWalk w2) {
					if (canFuse(w1, w2, snapshot)) {
						fusedSteps.add(new ScmWalk2Hop(w1.relationName(), w1.relationId(), w2.relationName(),
								w2.relationId()));
						i += 2;
						continue;
					}
	...
	private static boolean canFuse(ScmWalk w1, ScmWalk w2, ImpulseGraphSnapshot snapshot) {
		...
		if (w1.direction() != ScmWalk.Direction.FORWARD_CSR || !w1.shaderSteps().isEmpty()
				|| !w1.subSteps().isEmpty()) {
			return false;
		}
		if (w2.direction() != ScmWalk.Direction.FORWARD_CSR || !w2.shaderSteps().isEmpty()
				|| !w2.subSteps().isEmpty()) {
			return false;
		}
		...
		if (snapshot != null) {
			...
			if (stats != null) {
				...
				return stats.getAvgInDegree() <= OptimizerConfig.FUSED_2HOP_MAX_MULTIPLICITY_THRESHOLD || stats
						.getMultiplicity() == org.impulsegraph.api.stats.RelationStatistics.Multiplicity.ONE_TO_ONE
