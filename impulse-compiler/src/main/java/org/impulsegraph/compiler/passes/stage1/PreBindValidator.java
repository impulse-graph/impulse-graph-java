...
/**
 * Stage 1 Pass: Pre-bind syntactic and structural validation. Rejects malformed
 * queries, invalid operator arities, and syntax errors before physical snapshot
 * binding.
 */
		...
		if (node instanceof ScmProgram prog) {
			...
			if (walk.relationName().isEmpty() && walk.relationId() < 0) {
				throw new IllegalArgumentException(
						"Pre-Bind Validation Error: Walk step missing relation specification");
			}
