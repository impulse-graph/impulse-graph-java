...
public final class AlgebraicTypeInferencePass implements CompilerPass {
	...
	public CelAstNode inferCel(CelAstNode node, GraphStatistics stats) {
		...
		AlgebraicSignature sig = switch (nodeWithChildren.kind()) {
			...
			default -> nodeWithChildren.signature() != null
					? nodeWithChildren.signature()
					: AlgebraicSignature.defaultGeneral();
		};
	...
	private AlgebraicSignature inferMemberAccess(CelAstNode node, GraphStatistics stats) {
		...
		if (stats != null) {
			...
			if (attrStats != null) {
				...
				return new AlgebraicSignature(bound, MorphismClass.GENERAL, AlgebraicSignature.ALG_NONE,
						attrStats.monotonicity(), AlgebraicSignature.HOMO_NONE, true, false, false, 0, 0.0);
			}
		}
	...
	private AlgebraicSignature inferUnaryOp(CelAstNode node) {
		...
		if (childSig == null)
			childSig = AlgebraicSignature.defaultGeneral();
		...
		if ("-".equals(op) && childSig.interval().isBounded()) {
			...
			return new AlgebraicSignature(neg, MorphismClass.GENERAL, AlgebraicSignature.ALG_NONE, mono,
					AlgebraicSignature.HOMO_COMMUTES_WITH_MIN, // -max(X) == min(-X)
					childSig.isPure(), childSig.isConstantKnown(), childSig.constantBoolVal(),
					-childSig.constantIntVal(), -childSig.constantFloatVal());
		}
	...
	private AlgebraicSignature inferBinaryOp(CelAstNode node) {
		...
		if (left == null)
			left = AlgebraicSignature.defaultGeneral();
		if (right == null)
			right = AlgebraicSignature.defaultGeneral();
		...
		if (left.interval().isBounded() && right.interval().isBounded()) {
			...
			if ("+".equals(op)) {
				IntervalBound sum = new IntervalBound(l.minInt() + r.minInt(), l.maxInt() + r.maxInt(),
						l.minFloat() + r.minFloat(), l.maxFloat() + r.maxFloat(), true);
				return new AlgebraicSignature(sum, MorphismClass.GENERAL,
						AlgebraicSignature.ALG_SEMIGROUP | AlgebraicSignature.ALG_MONOID
								| AlgebraicSignature.ALG_COMMUTATIVE,
						combineMonotonicity(left.monotonicity(), right.monotonicity()),
						AlgebraicSignature.HOMO_COMMUTES_WITH_SUM, left.isPure() && right.isPure(),
						left.isConstantKnown() && right.isConstantKnown(), false,
			...
			if (">".equals(op)) {
				...
				if (l.maxInt() < r.minInt() || l.minInt() > r.maxInt())
					return AlgebraicSignature.ofConstantBool(false);
			} else if ("!=".equals(op)) {
				if (l.maxInt() < r.minInt() || l.minInt() > r.maxInt())
					return AlgebraicSignature.ofConstantBool(true);
		...
		if ("&&".equals(op)) {
			if ((left.isConstantKnown() && !left.constantBoolVal())
					|| (right.isConstantKnown() && !right.constantBoolVal())) {
				return AlgebraicSignature.ofConstantBool(false);
			}
			if (left.isConstantKnown() && left.constantBoolVal() && right.isConstantKnown()
					&& right.constantBoolVal()) {
				return AlgebraicSignature.ofConstantBool(true);
			}
		} else if ("||".equals(op)) {
			if ((left.isConstantKnown() && left.constantBoolVal())
	...
	private AlgebraicSignature inferFunctionCall(CelAstNode node) {
		...
		if (func.equals("log") || func.equals("exp") || func.equals("sqrt")) {
			return new AlgebraicSignature(IntervalBound.UNBOUNDED, MorphismClass.GENERAL, AlgebraicSignature.ALG_NONE,
		...
		if (func.equals("max") || func.equals("min") || func.equals("argmin") || func.equals("argmax")) {
			return new AlgebraicSignature(IntervalBound.UNBOUNDED, MorphismClass.GENERAL,
					AlgebraicSignature.ALG_SEMIGROUP | AlgebraicSignature.ALG_MONOID
							| AlgebraicSignature.ALG_COMMUTATIVE | AlgebraicSignature.ALG_IDEMPOTENT,
					Monotonicity.MONO_NONE, AlgebraicSignature.HOMO_NONE, true, false, false, 0, 0.0);
		}
	...
	private Monotonicity combineMonotonicity(Monotonicity a, Monotonicity b) {
		if (a == Monotonicity.MONO_CONSTANT)
			return b;
		if (b == Monotonicity.MONO_CONSTANT)
			return a;
		if (a == Monotonicity.MONO_STRICT_INC && b == Monotonicity.MONO_STRICT_INC)
			return Monotonicity.MONO_STRICT_INC;
		if (a == Monotonicity.MONO_STRICT_DEC && b == Monotonicity.MONO_STRICT_DEC)
			return Monotonicity.MONO_STRICT_DEC;
		return Monotonicity.MONO_NONE;
	}
}
