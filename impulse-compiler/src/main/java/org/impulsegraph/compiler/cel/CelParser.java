...
/**
 * Pure Java Pratt Recursive Descent Parser for Google Common Expression
 * Language (CEL). 1-to-1 equivalent with impulse-cpp Pratt Parser.
 */
public final class CelParser {

	public enum Precedence {
		PREC_NONE, PREC_CONDITIONAL, // ? :
		PREC_OR, // ||
		PREC_AND, // &&
		PREC_COMPARE, // == != < <= > >= in
		PREC_ADD_SUB, // + -
	...
	private CelAstNode parsePrecedence(Precedence prec) {
		...
		while (prec.ordinal() <= getPrecedence(curr.type()).ordinal()) {
			...
			if (op.type() == CelTokenType.QUESTION) {
				...
				Precedence nextPrec = nextPrecOrd < Precedence.values().length
						? Precedence.values()[nextPrecOrd]
						: Precedence.PREC_CALL_MEMBER;
