...
/**
 * SRE-Grade Query Registry managing pre-registered named prepared queries. Acts
 * as the authoritative validation gate for Blue/Green snapshot swaps.
 */
public final class QueryRegistry {
	...
	/**
	 * Pre-flight Blue/Green snapshot deployment gate: Compiles and validates all
	 * registered queries against the candidate snapshot. Throws
	 * IllegalStateException immediately if any query fails validation, aborting the
	 * swap.
	 */
	public Map<String, CompiledQuery> validateAndBindAll(ImpulseGraphSnapshot candidateSnapshot, Arena arena) {
		...
		if (!failures.isEmpty()) {
			throw new IllegalStateException("Blue/Green Swap Pre-Flight Gate Rejected: " + failures.size()
					+ " query validation failures detected:\n - " + String.join("\n - ", failures));
		}
