...
public final class CypherLexer {
	...
	public CypherToken nextToken() {
		...
		switch (c) {
			case '(' -> {
				return new CypherToken(CypherTokenType.LPAREN, "(", startPos, startLine, startCol);
			}
			case ')' -> {
				return new CypherToken(CypherTokenType.RPAREN, ")", startPos, startLine, startCol);
			}
			case '[' -> {
				return new CypherToken(CypherTokenType.LBRACKET, "[", startPos, startLine, startCol);
			}
			case ']' -> {
				return new CypherToken(CypherTokenType.RBRACKET, "]", startPos, startLine, startCol);
			}
			case ':' -> {
				return new CypherToken(CypherTokenType.COLON, ":", startPos, startLine, startCol);
			}
			case ',' -> {
				return new CypherToken(CypherTokenType.COMMA, ",", startPos, startLine, startCol);
			}
			case '.' -> {
				return new CypherToken(CypherTokenType.DOT, ".", startPos, startLine, startCol);
			}
			case '|' -> {
				return new CypherToken(CypherTokenType.PIPE, "|", startPos, startLine, startCol);
			}
			case '*' -> {
				return new CypherToken(CypherTokenType.STAR, "*", startPos, startLine, startCol);
			}
			...
			default -> {
				...
				return new CypherToken(CypherTokenType.ERROR, "Unexpected character: " + c, startPos, startLine,
						startCol);
			}
		}
	}
	...
	private CypherToken scanBacktickedIdentifier(int startPos, int startLine, int startCol) {
		...
		if (isAtEnd()) {
			return new CypherToken(CypherTokenType.ERROR, "Unterminated backtick identifier", startPos, startLine,
					startCol);
		}
	...
	private boolean match(char expected) {
		if (isAtEnd() || src.charAt(cursor) != expected)
			return false;
		cursor++;
		col++;
		return true;
