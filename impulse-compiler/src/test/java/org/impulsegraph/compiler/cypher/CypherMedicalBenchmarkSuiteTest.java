 ...
 * compiled through the 7-stage Impulse Compiler and executed over Hetionet and
 * DRKG.
 */
	...
	void testAllCypherMedicalQueries() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			System.out.println(
					"=========================================================================================================");
			System.out.println(
					"               DECLARATIVE OPENCYPHER MEDICAL KNOWLEDGE GRAPH BENCHMARK                                  ");
			System.out.println(
					"=========================================================================================================");
			System.out.println(
					" Frontend: openCypher (MATCH ... WHERE ... RETURN) -> ImpScheme AST -> 7-Stage Compiler -> impOps ISA   ");
			System.out.println(
					" Target Hardware: Apple Silicon M-Series | JVM: Java 25 (FFM & Vector API)                               ");
			System.out.println(
					" Iterations: 20,000 runs per query (10,000 warmup runs)                                                 ");
			System.out.println(
					"=========================================================================================================\n");
			...
			runCypherBenchmark("Cypher Q1: 4-Hop Pathway Drug Repurposing (CbGpPWpD)", "Hetionet v1.0", """
					MATCH (d:Disease)-[:DaG]->(g1:Gene)-[:GpPW]->(p:Pathway)<-[:GpPW]-(g2:Gene)<-[:CbG]-(c:Compound)
					WHERE d.id = $diseaseId
					RETURN c
					""", hetionet, diseaseSeed, arena);

			// 2. Cypher Q2: 2-Hop Expression Inversion / MoA
			runCypherBenchmark("Cypher Q2: 2-Hop Expression Counteraction / MoA (CuG<rGaD)", "Hetionet v1.0", """
					MATCH (d:Disease)-[:DdG]->(g:Gene)<-[:CuG]-(c:Compound)
					WHERE d.id = $diseaseId
					RETURN c
					""", hetionet, diseaseDdGSeed, arena);

			// 3. Cypher Q3: 2-Hop Chemical Resemblance
			runCypherBenchmark("Cypher Q3: 2-Hop Chemical Resemblance Transitivity (CrCtD)", "Hetionet v1.0", """
					MATCH (d:Disease)<-[:CtD]-(c1:Compound)-[:CrC]->(c2:Compound)
					WHERE d.id = $diseaseId
					RETURN c2
					""", hetionet, diseaseCtDSeed, arena);

			// 4. Cypher Q4: 3-Hop Shared Anatomy Pathology
			runCypherBenchmark("Cypher Q4: 3-Hop Shared Anatomy Pathology & Target Discovery (DlAeGbC)",
					"Hetionet v1.0", """
							MATCH (d:Disease)-[:DlA]->(a:Anatomy)-[:AeG]->(g:Gene)<-[:CbG]-(c:Compound)
							WHERE d.id = $diseaseId
							RETURN c
							""", hetionet, diseaseDlASeed, arena);

			// 5. Cypher Q5: 3-Hop Precision Oncology Signaling (DRKG)
			runCypherBenchmark("Cypher Q5: 3-Hop Precision Oncology Cascades (DRKG Multi-Source)",
					...
					"""
							...
							RETURN c
							""",
					drkg, drkgDiseaseSeed, arena);

			// 6. Cypher Q6: 2-Hop Adverse DDI Warning (DRKG)
			runCypherBenchmark("Cypher Q6: 2-Hop Polypharmacology Adverse DDI Warning (DRKG)",
					...
					"""
							...
							RETURN s
							""",
					drkg, drkgCompoundSeed, arena);
		}
	}

	private void runCypherBenchmark(String title, String dataset, String cypherQuery, ImpulseGraphSnapshot snapshot,
			int seedNode, Arena arena) {
