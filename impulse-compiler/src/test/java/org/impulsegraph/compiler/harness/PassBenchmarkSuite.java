 ...
 * Measures compilation latency and A/B pass speedups across un-optimized vs
 * optimized AST pipelines.
 */
public class PassBenchmarkSuite {
	...
	void benchmarkCompilerPipelineThroughput() {
		...
		try (Arena arena = Arena.ofConfined()) {
			...
			System.out.printf(
					"[Compiler Benchmark] 10,000 full pipeline compilations completed in %.2f ms (Avg: %.3f µs/compile, Throughput: %,.0f compiles/sec)%n",
	...
	private ImpScmNode runFullPipeline(CompilerContext ctx) {
		...
		ScmProgram raw = ScmProgram.of(ScmWalk.forward("users"), ScmCollect.distinct());
