 ...
 * Measures real-world execution speedup across 5 major compiler optimization
 * rules: 1. Zone Map Pruning (Dead-Code Scan Elimination) 2. Reverse CSC
 * Direction Selection vs Dense Forward CSR Walk 3. Virtual Relation Partition
 * Elimination (Selective Scan vs Scanning All Partitions) 4. Injective Path
 * Deduplication Bypass (Streaming BitSet vs Expensive HashSet Distinct) 5.
 * Monotonic Homomorphism Commutation (max(log(x)) -> log(max(x)))
 */
public class OptimizationAblationBenchmarkTest {
	...
	void benchZoneMapPruning() {
		...
		AttributeStatistics ageStats = new AttributeStatistics("age", 18, 114, 18.0, 114.0, "", "", 0, 96,
		...
		try (Arena arena = Arena.ofShared()) {
			...
			for (int i = 0; i <= nodeCount; i++)
				rows.setAtIndex(ValueLayout.JAVA_INT, i, i * 10);
			for (int e = 0; e < edgeCount; e++)
				cols.setAtIndex(ValueLayout.JAVA_INT, e, (e * 31) % nodeCount);
			for (int n = 0; n < nodeCount; n++)
				ageAttr.setAtIndex(ValueLayout.JAVA_INT, n, 18 + (n % 96));

			org.impulsegraph.storage.csr.RelationSnapshot rel = new org.impulsegraph.storage.csr.RelationSnapshot(arena,
					nodeCount, edgeCount, rows, cols, List.of(ageAttr));
			...
			printResult("1. Zone Map Dead-Code Pruning", unoptAvgUs, optAvgUs, speedup,
					"1M edges scanned vs 0 edges (Instant O(1) empty)");
		}
	}
	...
	void benchCscDirectionSelection() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			// UNOPTIMIZED (Forward CSR Scan): Must scan all 97k compounds to find which
			// treat targetDiseaseId
			long t0Unopt = System.nanoTime();
			int unoptFound = 0;
			for (int r = 0; r < runs; r++) {
				int found = 0;
			...
			// OPTIMIZED (Reverse CSC Walk): Direct O(degree) index lookup in CSC transpose
			// segment!
			long t0Opt = System.nanoTime();
			int optFound = 0;
			for (int r = 0; r < runs; r++) {
			...
			printResult("2. Reverse CSC vs Forward CSR", unoptAvgUs, optAvgUs, speedup,
					"Full forward scan vs O(degree) reverse CSC slice");
		}
	}
	...
	void benchPartitionElimination() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			// 3 Constituent relations: CbG (binds: 11,571 edges), CuG (upregulates: 18,756
			// edges), CdG (downregulates: 21,102 edges)
			...
			printResult("3. Partition Elimination", unoptAvgUs, optAvgUs, speedup,
					"Scanned 1 matching partition instead of all 3 partitions");
		}
	}
	...
	void benchInjectiveDeduplicationBypass() {
		...
		for (int i = 0; i < edgeCount; i++)
			streamTargets[i] = i * 2; // Injective: 0 duplicates

		int runs = 1_000;

		// UNOPTIMIZED: Standard DISTINCT requires allocating and inserting into a
		// HashSet / sorting
		long t0Unopt = System.nanoTime();
		int unoptUnique = 0;
		for (int r = 0; r < runs; r++) {
		...
		// OPTIMIZED: Injective Deduplication Bypass (Direct bitset word stream, 0
		// hashing, 0 allocations)
		long t0Opt = System.nanoTime();
		int optUnique = 0;
		for (int r = 0; r < runs; r++) {
		...
		printResult("4. Injective Deduplication Bypass", unoptAvgUs, optAvgUs, speedup,
				"Zero-allocation bitset stream vs HashSet insertion");
	}
	...
	void benchMonotonicHomomorphism() {
		...
		printResult("5. Monotonic Homomorphism", unoptAvgUs, optAvgUs, speedup,
				"1 transcendental log() call vs 100,000 calls in inner loop");
	}

	private static int scanRelationNeighbors(RelationSnapshot rel, int sourceNode) {
		if (sourceNode < 0 || sourceNode >= rel.getNodeCount())
			return 0;
		MemorySegment rowSeg = rel.getRowOffsetsSegment();
	...
	private static void printResult(String name, double unoptUs, double optUs, double speedup, String mechanism) {
		System.out.println(
				"---------------------------------------------------------------------------------------------------------");
		System.out.printf(" %-36s | Unopt: %9.3f µs | Opt: %8.3f µs | Speedup: %,7.1fx | %s%n", name, unoptUs, optUs,
				speedup, mechanism);
	}
}
