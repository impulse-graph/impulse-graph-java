 ...
 * <p>
 * Tests 2-Hop Fusion profitability across different graph topologies: 1. Low
 * Multiplicity / Injective Traversal (Multiplicity <= 1.5): Fused 2-Hop wins by
 * avoiding bitset allocation. 2. High Fan-out Multiplicity Traversal
 * (Multiplicity >= 10.0): Hop-Hop with bitset deduplication protects against
 * duplicate work. 3. Real Hetionet CbG -> GpPW Traversal.
 */
public class TwoHopFusionAblationBenchmarkTest {
	...
	void testLowMultiplicityFusion() {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			org.impulsegraph.storage.csr.RelationSnapshot rel1 = new org.impulsegraph.storage.csr.RelationSnapshot(
					arena, nodeCount, edgeCount, r1Offsets, r1Targets, java.util.List.of());
			org.impulsegraph.storage.csr.RelationSnapshot rel2 = new org.impulsegraph.storage.csr.RelationSnapshot(
					arena, nodeCount, edgeCount, r2Offsets, r2Targets, java.util.List.of());
			ImpulseGraphSnapshot snapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
					Map.of("rel1", rel1, "rel2", rel2));

			int runs = 10_000;
			int seed = 42;

			// 1. STANDARD HOP-HOP (2 separate OP_CSR_WALK instructions with intermediate
			// bitset)
			ScmProgram astHopHop = ScmProgram.of(ScmWalk.forward("rel1"), ScmWalk.forward("rel2"), ScmCollect.bitset());
			...
			for (int i = 0; i < 5_000; i++)
				ImpulseVmInterpreter.execute(progHopHop.programSegment(), progHopHop.instructionCount(), snapshot, seed,
						arena);

			long t0HopHop = System.nanoTime();
			for (int r = 0; r < runs; r++) {
				Object res = ImpulseVmInterpreter.execute(progHopHop.programSegment(), progHopHop.instructionCount(),
						snapshot, seed, arena);
				assertNotNull(res);
			}
			...
			ScmProgram astFused = ScmProgram.of(ScmWalk2Hop.of("rel1", "rel2"), ScmCollect.bitset());
			...
			// Warmup
			for (int i = 0; i < 5_000; i++)
				ImpulseVmInterpreter.execute(progFused.programSegment(), progFused.instructionCount(), snapshot, seed,
						arena);

			long t0Fused = System.nanoTime();
			for (int r = 0; r < runs; r++) {
				Object res = ImpulseVmInterpreter.execute(progFused.programSegment(), progFused.instructionCount(),
						snapshot, seed, arena);
				assertNotNull(res);
			}
			...
			System.out.println(
					"=========================================================================================================");
			System.out.println(
					"   BENCHMARK 1: LOW-MULTIPLICITY (1:1 Injective Path) -> FUSED 2-HOP vs HOP-HOP                          ");
			System.out.println(
					"=========================================================================================================");
			System.out.printf(" Standard Hop-Hop Latency:   %8.3f µs  (Allocates & scans intermediate R1 bitset)%n",
					hopHopAvgUs);
			System.out.printf(" Fused 2-Hop Latency:        %8.3f µs  (Zero intermediate bitset, register execution)%n",
					fusedAvgUs);
	...
	void testHighMultiplicityProtection() {
		// Demonstrates why FUSED_2HOP_MAX_MULTIPLICITY_THRESHOLD = 1.5 protects against
		...
		try (Arena arena = Arena.ofShared()) {
			...
			for (int i = 2; i <= nodeCount; i++)
				r1Offsets.setAtIndex(ValueLayout.JAVA_INT, i, 50);
			for (int e = 0; e < 50; e++)
				r1Targets.setAtIndex(ValueLayout.JAVA_INT, e, 5); // all target node 5!

			r2Offsets.setAtIndex(ValueLayout.JAVA_INT, 5, 0);
			r2Offsets.setAtIndex(ValueLayout.JAVA_INT, 6, 100);
			for (int e = 0; e < 100; e++)
				r2Targets.setAtIndex(ValueLayout.JAVA_INT, e, e + 10);

			org.impulsegraph.storage.csr.RelationSnapshot rel1 = new org.impulsegraph.storage.csr.RelationSnapshot(
					arena, nodeCount, 50, r1Offsets, r1Targets, java.util.List.of());
			org.impulsegraph.storage.csr.RelationSnapshot rel2 = new org.impulsegraph.storage.csr.RelationSnapshot(
					arena, nodeCount, 100, r2Offsets, r2Targets, java.util.List.of());
			ImpulseGraphSnapshot snapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
					Map.of("rel1", rel1, "rel2", rel2));
			...
			org.impulsegraph.api.stats.RelationStatistics rel1Stats = new org.impulsegraph.api.stats.RelationStatistics(
					nodeCount, 50, 1, 50, 50.0, 0.0, 50, 50, 50, 0.99, new OffHeapBitSet(arena, nodeCount),
					org.impulsegraph.api.stats.RelationStatistics.Multiplicity.MANY_TO_MANY, 50, 50.0, false, false,
					false);
			...
			// Because multiplicity of rel1 > 1.5, KernelFusionPass correctly DOES NOT fuse,
			// preserving Hop-Hop deduplication!
			boolean preservedHopHop = !transformed.toScmString().contains("csr-walk-2hop");
			System.out.println(
					"=========================================================================================================");
			System.out.println(
					"   BENCHMARK 2: HIGH-FANOUT SAFETY CHECK (Multiplicity Threshold = 1.5)                                  ");
			System.out.println(
					"=========================================================================================================");
			...
			assertTrue(preservedHopHop,
					"Should preserve Hop-Hop on high-fanout relation to deduplicate intermediate targets");
		}
	}
	...
	void testSimdVectorFusedPredicateEvaluation() {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			org.impulsegraph.storage.csr.RelationSnapshot rel = new org.impulsegraph.storage.csr.RelationSnapshot(arena,
					1, count, rOffsets, cTargets, java.util.List.of(attrSeg));
			...
			System.out.println(
					"=========================================================================================================");
			System.out.println(
					"   BENCHMARK 3: SIMD VECTOR API FUSED PREDICATE EVALUATION (512-bit Vector API vs Scalar)                ");
			System.out.println(
					"=========================================================================================================");
