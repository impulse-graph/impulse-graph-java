...
/**
 * Side-by-Side Architectural Benchmark: Mode A (Dynamic Re-compilation &
 * Specialization) vs Mode B (Prepared Query / Plan Caching) across 5,000,000
 * iterations (50,000 parameter values x 100 repetitions).
 */
public class ModeComparativeBenchmarkTest {
	...
	void compareModeAAndModeB() {
		AttributeStatistics mvaStats = new AttributeStatistics("mva_flow", 0, 0, 0.0, 1800.0, "", "", 0, 1354,
		...
		try (Arena arena = Arena.ofShared()) {
			...
			org.impulsegraph.storage.csr.RelationSnapshot branchRel = new org.impulsegraph.storage.csr.RelationSnapshot(
					arena, nodeCount, edgeCount, rowSeg, colSeg);
			ImpulseGraphSnapshot snapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
					Map.of("Branch", branchRel));
			...
			ScmProgram baseAst = ScmProgram.of(
					ScmWalk.forward("Branch",
							new ScmCelExpr("edge.status == 1 && edge.mva_flow > @maxRating", parsedTemplate)),
					ScmCollect.bitset());

			// Warmup Mode A
			CompilerOptions warmupOpts = CompilerOptions.builder().withTracing(false).withParameter("@maxRating", 250.0)
					.build();
			...
			for (int val = 1; val <= maxRatingLimit; val++) {
				CompilerOptions opts = CompilerOptions.builder().withTracing(false)
						.withParameter("@maxRating", (double) val).build();
			...
			ImpulseGraphQuery<ImpulseBitSet> query = ImpulseGraphQuery.<ImpulseBitSet>builder()
					.input("Bus", ArgType.SINGLE_NODE).walkEdge("Branch").collectBitSet();
			...
			System.out.println(
					"=================================================================================================");
			System.out.println(
					"                     IMPULSE GRAPH ENGINE: MODE A vs MODE B BENCHMARK                    ");
			System.out.println(
					"            (50,000 Parameter Values x 100 Repetitions = 5,000,000 Total Iterations)             ");
			System.out.println(
					"=================================================================================================");
			System.out.printf("%-30s | %-28s | %-28s%n", "Metric", "Mode A (Dynamic Re-compile)",
					"Mode B (Prepared Plan)");
			System.out.println(
					"-------------------------------+------------------------------+------------------------------");
			System.out.printf("%-30s | %-28s | %-28s%n", "Compilation Model", "Full 5-Pass Pipeline / Call",
					"Compiled ONCE (JIT MH / VM)");
			System.out.printf("%-30s | %,28.0f | %,28.0f%n", "Throughput (Ops / sec)", statsA.throughput,
					statsB.throughput);
			System.out.printf("%-30s | %25.3f ms | %25.3f ms%n", "Total Wall-Clock Time", statsA.wallClockMs,
					statsB.wallClockMs);
			System.out.println(
					"-------------------------------+------------------------------+------------------------------");
			System.out.printf("%-30s | %25.3f µs | %25.3f µs%n", "Min Latency", statsA.minNanos / 1000.0,
					statsB.minNanos / 1000.0);
			System.out.printf("%-30s | %25.3f µs | %25.3f µs%n", "p50 (Median Latency)", statsA.p50Nanos / 1000.0,
					statsB.p50Nanos / 1000.0);
			System.out.printf("%-30s | %25.3f µs | %25.3f µs%n", "Mean (Average Latency)", statsA.avgNanos / 1000.0,
					statsB.avgNanos / 1000.0);
			System.out.printf("%-30s | %25.3f µs | %25.3f µs%n", "p90 Latency", statsA.p90Nanos / 1000.0,
					statsB.p90Nanos / 1000.0);
			System.out.printf("%-30s | %25.3f µs | %25.3f µs%n", "p99 Latency", statsA.p99Nanos / 1000.0,
					statsB.p99Nanos / 1000.0);
			System.out.printf("%-30s | %25.3f µs | %25.3f µs%n", "p99.9 Latency", statsA.p999Nanos / 1000.0,
					statsB.p999Nanos / 1000.0);
			System.out.printf("%-30s | %25.3f µs | %25.3f µs%n", "Max Latency", statsA.maxNanos / 1000.0,
					statsB.maxNanos / 1000.0);
			System.out.println(
					"=================================================================================================");
		}
	}
	...
	private record TimingStats(double throughput, double wallClockMs, long minNanos, long p50Nanos, double avgNanos,
			long p90Nanos, long p99Nanos, long p999Nanos, long maxNanos) {
	}

	private static TimingStats computeStats(long[] sampleArray, int validSamples, long totalNanos, long elapsedNanos,
			int totalExecutions) {
