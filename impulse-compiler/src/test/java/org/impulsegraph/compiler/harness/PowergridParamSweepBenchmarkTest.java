...
/**
 * Parameter Sweep Benchmark for Power Grid Example 2 (Transmission Line MVA
 * Overload Alert): Evaluates maxRating parameter sweeps from 1 to 50,000 with
 * 100 repetitions per value, capturing fine-grained execution and compilation
 * timing statistics (min, mean, p50, p90, p99, p99.9, max).
 */
public class PowergridParamSweepBenchmarkTest {
	...
	void runPowergridParamSweepBenchmark() {
		...
		try (Arena arena = Arena.ofConfined()) {
			...
			// Benchmark execution over 50,000 distinct values with 100 repetitions per
			// value
			...
			// Sample 100,000 latency measurements across the run for high-resolution
			...
			for (int ratingVal = 1; ratingVal <= maxRatingLimit; ratingVal++) {
				...
				CompilerOptions opts = CompilerOptions.builder().withTracing(false)
						.withParameter("@maxRating", maxRating).build();
				...
				for (int r = 0; r < repsPerVal; r++) {
					...
					if (duration < minObservedNanos)
						minObservedNanos = duration;
					if (duration > maxObservedNanos)
						maxObservedNanos = duration;
			...
			System.out.println(
					"========================================================================================");
			System.out.println(
					"          POWERGRID PARAMETER SWEEP BENCHMARK (Example 2: maxRating 1..50,000)          ");
			System.out.println(
					"========================================================================================");
			System.out.printf(" Distinct Parameter Values:    %,d values (maxRating = 1.0 .. 50,000.0)%n",
					maxRatingLimit);
			...
			System.out.printf(" Total Wall-Clock Time:        %.3f ms (%.4f sec)%n",
					totalWallClockElapsedNanos / 1_000_000.0, totalSeconds);
