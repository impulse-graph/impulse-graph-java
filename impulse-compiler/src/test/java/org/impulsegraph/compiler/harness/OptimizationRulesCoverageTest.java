 ...
 * Contains 2-3 dedicated unit tests per optimization rule, inspecting the exact
 * AST output after every pass.
 */
public class OptimizationRulesCoverageTest {
	...
	class Rule2MonotonicHomomorphismTests {
		...
		void testMeetJoinInversion() {
			...
			CelAstNode optMaxNeg = MonotonicHomomorphismPass.INSTANCE
					.optimizeCel(AlgebraicTypeInferencePass.INSTANCE.inferCel(maxNeg, new GraphStatistics()));
			assertEquals("-", optMaxNeg.text());
			assertEquals("min", optMaxNeg.children().get(0).text());

			// min(-v) -> -max(v)
			CelAstNode minNeg = CelParser.parse("min(-v)");
			CelAstNode optMinNeg = MonotonicHomomorphismPass.INSTANCE
					.optimizeCel(AlgebraicTypeInferencePass.INSTANCE.inferCel(minNeg, new GraphStatistics()));
	...
	class Rule3InjectiveBypassTests {
		...
		void testMultiHopInjectiveCompositionBypass() {
			try (Arena arena = Arena.ofConfined()) {
				...
				org.impulsegraph.storage.csr.RelationSnapshot rel1 = createMockRelation(arena, rowOffsets1,
						colTargets1);

				int[] rowOffsets2 = {0, 1, 2, 3, 4};
				int[] colTargets2 = {10, 11, 12, 13};
				org.impulsegraph.storage.csr.RelationSnapshot rel2 = createMockRelation(arena, rowOffsets2,
						colTargets2);

				ImpulseGraphSnapshot snapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
						Map.of("step1", rel1, "step2", rel2));
		...
		void testManyToManyPreservesDistinct() {
			try (Arena arena = Arena.ofConfined()) {
				...
				org.impulsegraph.storage.csr.RelationSnapshot rel = createMockRelation(arena, rowOffsets, colTargets);
				ImpulseGraphSnapshot snapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
						Map.of("userFriends", rel));

				assertFalse(rel.getStatistics().isInjective());

				ScmProgram ast = ScmProgram.of(ScmWalk.forward("userFriends"), ScmCollect.distinct());
	...
	class Rule4VirtualRelationTests {
		...
		void testCoproductDecomposition() {
			try (Arena arena = Arena.ofConfined()) {
				...
				ImpulseGraphSnapshot snapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
						Map.of("in_section_fruit", secFruit, "in_section_bread", secBread));

				ScmProgram ast = ScmProgram.of(ScmWalk.forward("in_section"), ScmCollect.bitset());
		...
		void testThreeWayCoproductDecomposition() {
			try (Arena arena = Arena.ofConfined()) {
				ImpulseGraphSnapshot snapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
						Map.of("in_section_fruit", createMockRelation(arena, 1, 1), "in_section_bread",
								createMockRelation(arena, 1, 1), "in_section_dairy", createMockRelation(arena, 1, 1)));
	...
	class Rule5ParameterBindingTests {
		...
		void testStringParameterBinding() {
			...
			ScmProgram ast = ScmProgram.of(ScmWalk.forward("items", new ScmCelExpr("section == @P1", cel)),
					ScmCollect.bitset());

			CompilerOptions opts = CompilerOptions.builder().withParameter("@P1", "FRUIT").build();
		...
		void testNumericDoubleParameterBinding() {
			...
			ScmProgram ast = ScmProgram.of(ScmWalk.forward("bus", new ScmCelExpr("node.vm < @minVoltage", cel)),
					ScmCollect.bitset());

			CompilerOptions opts = CompilerOptions.builder().withParameter("@minVoltage", 0.95).build();
			CompilerContext ctx = new CompilerContext(null, opts, new PassTracer(opts));
		...
		void testCompoundExpressionMultipleParameters() {
			...
			CompilerOptions opts = CompilerOptions.builder().withParameter("@st", 1).withParameter("@minAge", 21)
					.build();
			CompilerContext ctx = new CompilerContext(null, opts, new PassTracer(opts));
	...
	private static org.impulsegraph.storage.csr.RelationSnapshot createMockRelation(Arena arena, int[] rowOffsets,
			int[] colTargets) {
		...
		return new org.impulsegraph.storage.csr.RelationSnapshot(arena, rowOffsets.length - 1, colTargets.length,
				rowSeg, colSeg);
	}
}
