...
/**
 * Test Harness for Universal Algebraic Compiler Optimizations. Verifies
 * mathematical equivalence and correctness of algebraic passes: - Zone Map
 * Pruning & Dead Code Elimination - Monotonic Homomorphism Pushdown
 * (max(log(x)) -> log(max(x))) - Injective Deduplication Bypass - Virtual
 * Super-Relation Coproduct Decomposition
 */
public class DifferentialCorrectnessHarnessTest {
	...
	void testZoneMapPruning() {
		...
		try (Arena arena = Arena.ofConfined()) {
			...
			ScmProgram ast = ScmProgram.of(ScmWalk.forward("userToGroup", new ScmCelExpr("node.age > 250", parsedCel)),
					ScmCollect.bitset());
	...
	void testVirtualRelationDecomposition() {
		try (Arena arena = Arena.ofConfined()) {
			...
			org.impulsegraph.storage.csr.RelationSnapshot secFruit = new org.impulsegraph.storage.csr.RelationSnapshot(
					arena, 1, 1, rows, cols);
			org.impulsegraph.storage.csr.RelationSnapshot secBread = new org.impulsegraph.storage.csr.RelationSnapshot(
					arena, 1, 1, rows, cols);
