...
/**
 * Showcase of Complex Biomedical & Pharmacological Graph Queries over Hetionet
 * v1.0 and DRKG with detailed Cold vs Warm Compilation and Off-Heap Traversal
 * Timing Profiles.
 */
public class HetionetAndDrkgQueryShowcaseTest {
	...
	void testDrugRepurposingMetapath() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			ScmProgram ast = ScmProgram.of(ScmWalk.forward("CtD"),
			...
			CompilerOptions coldOpts = CompilerOptions.builder().withTracing(true).withParameter("@minConfidence", 0.85)
					.build();
			...
			CompilerOptions warmOpts = CompilerOptions.builder().withTracing(false)
					.withParameter("@minConfidence", 0.85).build();
			...
			System.out.println(
					"========================================================================================");
			System.out.println(
					"   HETIONET QUERY 1: DRUG REPURPOSING METAPATH (Compound -> Disease -> Gene -> Pathway) ");
			System.out.println(
					"========================================================================================");
			System.out.printf(" Dataset Scope:                  47,031 nodes | 24 relations | 2,250,197 edges%n");
			System.out.printf(" Initial Off-Heap Stats Scan:    %.3f ms (one-time lazy scan of 2.25M edges)%n",
					statsMs);
			System.out.printf(" Cold First-Run Compilation:     %.3f ms (includes AST parsing & verification)%n",
					coldCompileMs);
			System.out.println(
					"----------------------------------------------------------------------------------------");
	...
	void testDrkgPrecisionTargetInhibitorDiscovery() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			ScmProgram ast = ScmProgram.of(ScmWalk.reverse("DRUGBANK::treats::Compound:Disease"),
					ScmWalk.forward("DGIDB::INHIBITOR::Gene:Compound",
							new ScmCelExpr("edge.potency_ic50 < @maxIc50Nm", parsedCel)),
					ScmCollect.bitset());

			// Cold First-Run Compilation
			CompilerOptions coldOpts = CompilerOptions.builder().withTracing(true).withParameter("@maxIc50Nm", 50.0)
					.build();
			PassTracer coldTracer = new PassTracer(coldOpts);
			CompilerContext coldCtx = new CompilerContext(graph, coldOpts, coldTracer);
			...
			// Warm Benchmark: 50,000 compilations
			CompilerOptions warmOpts = CompilerOptions.builder().withTracing(false).withParameter("@maxIc50Nm", 50.0)
					.build();
			CompilerContext warmCtx = new CompilerContext(graph, warmOpts, new PassTracer(warmOpts));
			...
			System.out.println(
					"========================================================================================");
			System.out.println(
					"   DRKG QUERY 3: TARGET INHIBITOR DISCOVERY (Disease -> Reverse CSC -> Forward CSR)     ");
			System.out.println(
					"========================================================================================");
			System.out.printf(" Dataset Scope:                  97,238 nodes | 107 relations | 5,874,261 edges%n");
			System.out.printf(
					" Initial Off-Heap Stats Scan:    %.3f ms (one-time scan of 107 relations & 5.87M edges)%n",
					statsMs);
			System.out.printf(" Cold First-Run Compilation:     %.3f ms (includes AST parsing & verification)%n",
					coldCompileMs);
			System.out.println(
