...
/**
 * Standard / Canonical Benchmark Queries on Medical Knowledge Graphs (Hetionet
 * & DRKG). Measures high-precision timing (Mean, P50, P90, P99, Throughput QPS)
 * across standard Project Rephetio (Neo4j Depot) and Amazon DRKG / DGL
 * benchmark queries.
 */
	...
	void runAllCanonicalBenchmarkQueries() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			System.out.println(
					"=========================================================================================================");
			System.out.println(
					"                  CANONICAL MEDICAL KNOWLEDGE GRAPH BENCHMARK SUITE                                     ");
			System.out.println(
					"=========================================================================================================");
			System.out.println(
					" Hardware: Apple Silicon M-Series | JVM: Java 25 (FFM & Vector API) | Storage: Zero-Copy MMAP .imps    ");
			System.out.println(
					" Benchmark Iterations: 20,000 runs per query (10,000 warmup runs)                                       ");
			System.out.println(
					"=========================================================================================================\n");
			...
			int diseaseCtDSeed = findActiveSeedNode(hetionet, "CtD", true); // Disease treated by compounds (reverse
																			// CtD)
			...
			// Disease -> (DaG forward) -> Gene -> (GpPW forward) -> Pathway -> (GpPW
			// reverse) -> Gene -> (CbG reverse) -> Compound
			runBenchmarkQuery("Q1: Pathway-Based Drug Repurposing (CbGpPWpD Metapath)", "Hetionet v1.0",
					"Disease -> Gene -> Pathway -> Gene -> Compound (4 Hops)", hetionet,
					ScmProgram.of(ScmWalk.forward("DaG"), ScmWalk.forward("GpPW"), ScmWalk.reverse("GpPW"),
							ScmWalk.reverse("CbG"), ScmCollect.bitset()),
					diseaseSeed, arena);

			// QUERY 2: Metapath CuG<rGaD (Expression Inversion / Mechanism-of-Action)
			// Disease -> (DdG forward) -> Downregulated Genes -> (CuG reverse) ->
			// Upregulating Compounds
			runBenchmarkQuery("Q2: Expression Counteraction / Mechanism-of-Action (CuG<rGaD)", "Hetionet v1.0",
					"Disease -> Downregulated Genes -> Upregulating Compounds (2 Hops)", hetionet,
					ScmProgram.of(ScmWalk.forward("DdG"), ScmWalk.reverse("CuG"), ScmCollect.bitset()), diseaseDdGSeed,
					arena);

			// QUERY 3: Metapath CrCtD (Chemical Structure Resemblance Transitivity)
			// Disease -> (CtD reverse) -> Known Treats Compounds -> (CrC forward) ->
			// Resembling Candidate Compounds
			runBenchmarkQuery("Q3: Chemical Structure Resemblance Transitivity (CrCtD)", "Hetionet v1.0",
					"Disease -> Known Treats Drugs -> Resembling Drugs (2 Hops)", hetionet,
					ScmProgram.of(ScmWalk.reverse("CtD"), ScmWalk.forward("CrC"), ScmCollect.bitset()), diseaseCtDSeed,
					arena);

			// QUERY 4: Metapath DaAeG&CtD (Shared Anatomy Pathology & Expression)
			// Disease -> (DlA forward) -> Anatomy -> (AeG forward) -> Expressed Genes ->
			// (CbG reverse) -> Targeting Compounds
			runBenchmarkQuery("Q4: Shared Anatomy Pathology & Target Discovery (DlAeGbC)", "Hetionet v1.0",
					"Disease -> Affected Anatomy -> Expressed Genes -> Targeting Drugs (3 Hops)", hetionet,
					ScmProgram.of(ScmWalk.forward("DlA"), ScmWalk.forward("AeG"), ScmWalk.reverse("CbG"),
							ScmCollect.bitset()),
					diseaseDlASeed, arena);

			// -------------------------------------------------------------
			// DRKG BENCHMARKS (124.41 MB | 97,238 nodes | 5.87M edges)
			// -------------------------------------------------------------
			// QUERY 5: Precision Oncology Upstream Kinase Cascades
			// Disease -> (DISGENET::da forward) -> Genes -> (STRING::interacts_with
			// forward) -> Interacting Genes -> (DRUGBANK::target reverse) -> Kinase
			// Inhibitors
			runBenchmarkQuery("Q5: Precision Oncology Upstream Signaling Cascades (Multi-Source)",
					"DRKG (DisGeNET + STRING + DrugBank)",
					"Disease -> Mutated Genes -> PPI Network -> Drug Targets -> Inhibitors (3 Hops)", drkg,
					ScmProgram.of(ScmWalk.forward("DISGENET::da"), ScmWalk.forward("STRING::interacts_with"),
							ScmWalk.reverse("DRUGBANK::target"), ScmCollect.bitset()),
					drkgDiseaseSeed, arena);

			// QUERY 6: Polypharmacology Adverse Drug-Drug Interaction (DDI) Warning
			// Compound -> (DRUGBANK::ddi_interactor_in forward) -> Interacting Drug ->
			// (GNBR::C forward) -> Side Effects
			runBenchmarkQuery("Q6: Polypharmacology Adverse Drug-Drug Interaction Warning",
					"DRKG (DrugBank DDI + GNBR Side Effects)",
					"Compound -> DDI Interactors -> Shared Severe Side Effects (2 Hops)", drkg,
					ScmProgram.of(ScmWalk.forward("DRUGBANK::ddi_interactor_in"), ScmWalk.forward("GNBR::C"),
							ScmCollect.bitset()),
					drkgCompoundSeed, arena);

			System.out.println(
					"=========================================================================================================");
			System.out.println(
					"                             ALL 6 BENCHMARKS COMPLETED ACCURATELY                                       ");
			System.out.println(
					"=========================================================================================================");
		}
	}

	private void runBenchmarkQuery(String title, String dataset, String description, ImpulseGraphSnapshot snapshot,
			ScmProgram ast, int seedNode, Arena arena) {
		...
		for (int r = 0; r < BENCHMARK_ITERS; r++) {
			...
			Object res = ImpulseVmInterpreter.execute(prog.programSegment(), prog.instructionCount(), snapshot,
					seedNode, arena);
