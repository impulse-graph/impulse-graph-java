...
/**
 * Showcase of Complex Power Grid Stability and Transmission Analysis Queries
 * with CEL Expressions and Dynamic Parameter Binding
 * (@minVoltage, @maxVoltage, @maxRating, @criticalMva).
 */
public class PowergridCelQueryShowcaseTest {
	...
	void testCriticalBusVoltageViolationQuery() {
		...
		AttributeStatistics vmStats = new AttributeStatistics("vm", 0, 0, 0.90, 1.12, "", "", 0, 1354,
		...
		try (Arena arena = Arena.ofConfined()) {
			...
			ImpulseGraphQuery<ImpulseBitSet> query = ImpulseGraphQuery.<ImpulseBitSet>builder().walkEdge("Branch")
					.filterWithCel("node.vm < @minVoltage || node.vm > @maxVoltage").bindParameter("@minVoltage", 0.95)
					.bindParameter("@maxVoltage", 1.05).collectBitSet();
			...
			CompilerOptions opts = CompilerOptions.builder().withTracing(true).withParameters(query.getParameters())
					.build();
			...
			ScmProgram rawAst = ScmProgram.of(
					ScmWalk.forward("Branch",
							new ScmCelExpr("node.vm < @minVoltage || node.vm > @maxVoltage", parsedCel)),
					ScmCollect.bitset());
	...
	void testTransmissionLineThermalOverloadQuery() {
		try (Arena arena = Arena.ofConfined()) {
			...
			ImpulseGraphQuery<ImpulseBitSet> query = ImpulseGraphQuery.<ImpulseBitSet>builder()
					...
					.bindParameter("@maxRating", 250.0).collectBitSet();
			...
			ScmProgram rawAst = ScmProgram.of(
					ScmWalk.forward("Branch",
							new ScmCelExpr("edge.status == 1 && edge.mva_flow > @maxRating", parsedCel)),
					ScmCollect.bitset());
	...
	void testN1ContingencyMultiHopReachability() {
		try (Arena arena = Arena.ofConfined()) {
			...
			// Multi-Hop Transitive Islanding Frontier over closed breakers (edge.status ==
			// 1)
			ImpulseGraphQuery<ImpulseBitSet> query = ImpulseGraphQuery.<ImpulseBitSet>builder()
					.repeatUntilStable(
							b -> b.walkEdgeWithCel("Branch", "edge.status == 1 && edge.x_pu < @maxReactance"))
					.bindParameter("@maxReactance", 0.15).collectBitSet();
