...
/**
 * Massive, comprehensive test suite spanning 40+ permutations of CEL
 * expressions, state projections, type scopes, validations, and edge cases.
 */
public class CelStateExpressionExtensiveTest {
	...
	void testCommasInsideParens() {
		List<ProjectionAstNode> nodes = ProjectionParser
				.parse("state.score:MAX = math.max(src.score, edge.score), state.id:ANY = src.id");
	...
	void testMemberLogic() {
		...
		assertEquals("(mask-and (vec-cmp-gt (get-attr src \"val\") 10) (<= (get-attr edge \"cost\") 5))",
				CelCompiler.toImpScheme(ast));
	}
	...
	void testComplexStreamFilter() {
		...
		assertEquals(
				"(stream-logic-and (stream-cmp-gt (stream-load-attr src \"age\") 21) (stream-cmp-eq (stream-load-attr edge \"valid\") #t))",
				ir);
	}
