...
public class HetionetScreen1AllDiseasesJmhBenchmark {
	...
	public void screen1_single_disease_point_query(Blackhole bh) {
		Object res = ImpulseVmInterpreter.execute(prog.programSegment(), prog.instructionCount(), hetionet,
				singleSeedDisease, arena);
		bh.consume(res);
	}
	...
	public void screen1_all_134_diseases_sequential_sweep(Blackhole bh) {
		for (int d : allActiveDiseases) {
			Object res = ImpulseVmInterpreter.execute(prog.programSegment(), prog.instructionCount(), hetionet, d,
					arena);
			bh.consume(res);
		}
	}
	...
	public static void main(String[] args) throws Exception {
		Options opt = new OptionsBuilder().include(HetionetScreen1AllDiseasesJmhBenchmark.class.getSimpleName())
				.forks(1).build();
		new Runner(opt).run();
	}
}
