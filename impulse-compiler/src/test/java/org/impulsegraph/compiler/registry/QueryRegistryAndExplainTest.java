...
/**
 * Tests for QueryRegistry, Blue/Green pre-flight validation gate, and
 * QueryExplainer.
 */
public class QueryRegistryAndExplainTest {
	...
	void testRegistryAndBlueGreenGate() {
		...
		ScmProgram queryAst = ScmProgram.of(ScmWalk.forward("userToGroup"), ScmCollect.bitset());
		...
		try (Arena arena = Arena.ofConfined()) {
			...
			org.impulsegraph.storage.csr.RelationSnapshot rel = new org.impulsegraph.storage.csr.RelationSnapshot(arena,
					1, 1, rowSeg, colSeg);
			ImpulseGraphSnapshot validSnapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
			...
			ImpulseGraphSnapshot invalidSnapshot = new org.impulsegraph.storage.csr.GraphSnapshot(arena,
					Map.of("otherRelation", rel));
