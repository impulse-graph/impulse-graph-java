...
public class VmExecutionEngineTest {
	...
	public void testSimpleNodeInputAndWalk() throws Throwable {
		try (Arena arena = Arena.ofShared()) {
			...
			// 2: OP_CSR_WALK (dst=2, payload=(1 << 16) | 0) -> R1 bitset {1} -> R2 bitset
			// {2}
	...
	public void testSetOperations() throws Throwable {
		try (Arena arena = Arena.ofShared()) {
			...
			// 1: OP_INIT_INPUT_SET (dst=1) -> R1 = set2 (from code, wait: we use init input
			// set for set1)
	...
	public void testVmInitializationAndSingleOpHalt() throws Throwable {
		try (Arena arena = Arena.ofConfined()) {
			...
			try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
				...
				// Verify initial VM state layout baseline (all 64 registers, types, flags, PC,
				// call stack)
				...
				for (int i = 0; i < 64; i++) {
					assertEquals(0L, VmHandlers.getRegisterValue(state, i),
							"Register R" + i + " must be initialized to 0");
					assertEquals(TYPE_NULL, VmHandlers.getRegisterType(state, i),
							"Register R" + i + " type must be TYPE_NULL");
				}

				for (int i = 0; i < 8; i++) {
					assertEquals(0, (int) CALL_STACK_ELEMENT_HANDLE.get(state, 0L, (long) i),
							"Call stack slot " + i + " must be 0");
				}
				...
				assertTrue(result instanceof ImpulseBitSet,
						"Single OP_HALT program returns default empty ImpulseBitSet");
				assertTrue(((ImpulseBitSet) result).isEmpty(),
						"Single OP_HALT program should return empty ImpulseBitSet");
			}
		}
	}
}
