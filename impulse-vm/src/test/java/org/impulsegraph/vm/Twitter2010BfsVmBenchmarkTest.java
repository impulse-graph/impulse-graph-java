...
public class Twitter2010BfsVmBenchmarkTest {
	...
	private static final Path TWITTER_SNAPSHOT_PATH = Path
			.of("/Users/jesse/impulse/datasets/twitter-2010/twitter-2010.csc.imps");
	...
	public void runTwitter2010BfsBenchmark() throws Throwable {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			RelationSnapshot rel = (org.impulsegraph.storage.csr.RelationSnapshot) graph.getAllRelationSnapshots()
			...
			if (!rel.hasCsc()) {
				...
				MemorySegment[] cscSegs = DefaultSnapshotBuilder.computeCscSegments(arena, nodeCount, (int) edgeCount,
						rel.getRowOffsetsSegment(), rel.getColumnTargetsSegment());
				rel.setCscSegments(cscSegs[0], cscSegs[1]);
				System.out.printf("CSC transpose index generated off-heap in %.2f ms%n",
						(System.nanoTime() - t0Csc) / 1_000_000.0);
			}

			// 1. Parallel Direction-Optimizing Lock-Free BFS Traversal (Root 613, degree
			// 2,997,469 - 10 Threads)
			int rootNode = 613;
						...
						java.util.stream.IntStream.range(0, numThreads).parallel().forEach(t -> {
							while (true) {
								...
								for (int v = startV; v < endV; v++) {
									if (visited[v] == 0) {
										int start = cscRowOff
												.getAtIndex(java.lang.foreign.ValueLayout.JAVA_INT_UNALIGNED, v);
										int end = cscRowOff.getAtIndex(java.lang.foreign.ValueLayout.JAVA_INT_UNALIGNED,
												v + 1);
										for (int idx = start; idx < end; idx++) {
											int u = cscColIdx
													.getAtIndex(java.lang.foreign.ValueLayout.JAVA_INT_UNALIGNED, idx);
											if (curFrontier[u] != 0) {
												visited[v] = 1;
												nextFrontier[v] = 1;
												break;
			...
			System.out.printf("Execution Time:                    %.3f ms (%.2fs)%n", csrOnlyTimeMs,
					csrOnlyTimeMs / 1000.0);
			...
			System.out.printf("Execution Time:                    %.3f ms (%.1f us)%n", microQueryTimeMs,
					microQueryTimeMs * 1000.0);
