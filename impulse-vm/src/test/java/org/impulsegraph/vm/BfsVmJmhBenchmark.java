...
public class BfsVmJmhBenchmark {
    ...
    public void profileBfsBottlenecks() throws Exception {
        ...
        try (Arena arena = Arena.ofShared()) {
            ...
            RelationSnapshot rel = (org.impulsegraph.storage.csr.RelationSnapshot) graph.getAllRelationSnapshots().values().iterator().next();
            int nodeCount = rel.getNodeCount();
            long edgeCount = (int) rel.getEdgeCount();

            if (!rel.hasCsc()) {
                MemorySegment[] csc = org.impulsegraph.storage.csr.DefaultSnapshotBuilder.computeCscSegments(
                        arena, rel.getNodeCount(), (int) rel.getEdgeCount(), rel.getRowOffsetsSegment(), rel.getColumnTargetsSegment()
                );
