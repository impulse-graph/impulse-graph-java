 ...
 * Measures phase-level latency for Push (CSR), Pull (CSC), and Set Math
 * operations.
 */
public class BfsVmJmhBenchmark {
	...
	public void profileBfsBottlenecks() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			long edgeCount = (int) rel.getEdgeCount();

			if (!rel.hasCsc()) {
				MemorySegment[] csc = org.impulsegraph.storage.csr.DefaultSnapshotBuilder.computeCscSegments(arena,
						rel.getNodeCount(), (int) rel.getEdgeCount(), rel.getRowOffsetsSegment(),
			...
			for (int warmup = 0; warmup < 5; warmup++) {
				try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
					...
					int fSize = 1;
					VmHandlers.Instruction adaptiveInstr = new VmHandlers.Instruction(VmRegisterType.OP_ADAPTIVE_WALK,
							(byte) 0, 4, 2 | (3 << 16) | (0 << 24));
					while (fSize > 0) {
