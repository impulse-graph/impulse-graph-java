 ...
 * Executes sub-microsecond electrical power grid islanding detection via
 * Impulse VM OP_ISLAND_DETECT.
 */
public class PowerGridContingencyVmBenchmarkTest {
	...
	public void runPowerGridPegase1354ContingencyBenchmark() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			ImpulseGraphSnapshot graph = BinarySnapshotLoader.loadSnapshot(snapshotPath, arena).graph();
			...
			long lineCount = rel.getEdgeCount();
			...
			VmStateLayout.INSTR_OPCODE_HANDLE.set(prog, 1 * VmStateLayout.INSTRUCTION_SIZE_BYTES,
					VmRegisterType.OP_HALT);
			...
			int n2Evaluations = (int) Math.min(50_000, lineCount * (lineCount - 1) / 2);
			...
			System.out.printf("Critical Grid Islanding Events:      %,d critical paired outages%n",
					n2IslandingEvents.get());
