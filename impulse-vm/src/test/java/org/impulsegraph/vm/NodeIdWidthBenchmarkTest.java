...
public class NodeIdWidthBenchmarkTest {
    ...
    public void runTwitter2010NodeIdWidthBenchmark() throws Throwable {
        ...
        try (Arena arena = Arena.ofShared()) {
            ...
            double size32Gb = (rel32.getColumnTargetsSegment().byteSize() + rel32.getRowOffsetsSegment().byteSize()) / (1024.0 * 1024.0 * 1024.0);
            ...
            System.out.printf("Allocating 64-bit target array:       %.2f GB (8 bytes/node ID)...%n", targets64ByteSize / (1024.0 * 1024.0 * 1024.0));
            ...
            double size64Gb = (targets64.byteSize() + rel32.getRowOffsetsSegment().byteSize()) / (1024.0 * 1024.0 * 1024.0);
            System.out.printf("64-bit Topology Buffer Memory Size:   %.2f GB (8 bytes/node ID)%n%n", size64Gb);

            RelationSnapshot rel64 = new RelationSnapshot(
                    arena,
                    nodeCount,
                    (int) edgeCount,
                    rel32.getRowOffsetsSegment(),
                    targets64,
                    rel32.getCscRowOffsetsSegment(),
                    rel32.getCscColumnTargetsSegment(),
                    java.util.Collections.<MemorySegment>emptyList(),
                    java.util.Collections.<MemorySegment>emptyList(),
                    (byte) 4, // srcNodeIdWidth = 4 bytes
            ...
            // Test across high-degree hub nodes (e.g. node 613, degree ~3,000,000, and node 12, degree ~2,000,000)
            ...
            int iterations = 1000;
            System.out.println("-----------------------------------------------------------------------------------------");
            System.out.println(" BENCHMARK RESULTS: 1000 CSR TRAVERSAL ITERATIONS OVER HIGH-DEGREE HUBS                 ");
            System.out.println("-----------------------------------------------------------------------------------------");
            ...
            System.out.printf("   • Memory Footprint:           %.2f GB (16 node IDs / 64-byte L1 cache line)%n", size32Gb);
            ...
            System.out.printf("   • Memory Footprint:           %.2f GB (8 node IDs / 64-byte L1 cache line)%n", size64Gb);
            ...
            System.out.printf("   • 64-bit Node IDs suffer a %.1f%% Latency Penalty (L1/L2 Cache Pressure & Bandwidth)%n", ((avgLatency64Us - avgLatency32Us) / avgLatency32Us) * 100.0);
            System.out.printf("   • Memory Bandwidth Efficiency Gain: +%.0f%% denser cache line packing%n", cacheEfficiencyGain);
            System.out.println("=========================================================================================\n");
        }
    }
}
