...
class ImpulseQueryBuilderTest {
	...
	void testBuildUserToRoleQuery() {
		ImpulseGraphQuery<Object> query = ImpulseGraphQuery.builder().input("USER", ArgType.ROARING_BITSET)
				.walkEdge("userToGroup").walkEdge("groupToRole").collect(ReturnType.ROARING_BITSET);
	...
	void testBuildTransitiveClosureQuery() {
		ImpulseGraphQuery<Object> query = ImpulseGraphQuery.builder().input("ORG", ArgType.SINGLE_NODE)
				.repeatUntilStable(step -> step.walkEdge("orgToParentOrg")).collect(ReturnType.ROARING_BITSET);
	...
	void testBuildBattingAverageQuery() {
		ImpulseGraphQuery<Object> query = ImpulseGraphQuery.builder().input("Player", ArgType.SINGLE_NODE)
				.walkEdgeWithState("PLAYED_IN",
						"state.total_hits:SUM = edge.hits, state.total_at_bats:SUM = edge.at_bats")
