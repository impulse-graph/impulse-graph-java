...
public class VmStateLayoutTest {
	...
	public void testStateSegmentReadWrite() {
		try (Arena arena = Arena.ofShared(); VmQueryContext ctx = new VmQueryContext(null, arena)) {
			...
			assertEquals(VmRegisterType.FLAG_ZF | VmRegisterType.FLAG_EQ,
					(long) VmStateLayout.FLAGS_HANDLE.get(state, 0L));
			...
			assertEquals(VmRegisterType.TYPE_NODE_ID,
					(byte) VmStateLayout.REGISTER_TYPE_ELEMENT_HANDLE.get(state, 0L, 0L));

			assertEquals(9999L, (long) VmStateLayout.REGISTER_ELEMENT_HANDLE.get(state, 0L, 63L));
			assertEquals(VmRegisterType.TYPE_INT64,
					(byte) VmStateLayout.REGISTER_TYPE_ELEMENT_HANDLE.get(state, 0L, 63L));
	...
	public void testStateSegmentInitializationBaseline() {
		try (Arena arena = Arena.ofShared(); VmQueryContext ctx = new VmQueryContext(null, arena)) {
			...
			assertEquals(0, (int) VmStateLayout.CALL_STACK_DEPTH_HANDLE.get(state, 0L),
					"Initial call stack depth MUST be 0");

			for (int r = 0; r < 64; r++) {
				assertEquals(0L, (long) VmStateLayout.REGISTER_ELEMENT_HANDLE.get(state, 0L, (long) r),
						"Register R" + r + " value MUST be 0");
				assertEquals(VmRegisterType.TYPE_NULL,
						(byte) VmStateLayout.REGISTER_TYPE_ELEMENT_HANDLE.get(state, 0L, (long) r),
						"Register R" + r + " type MUST be TYPE_NULL");
			}

			for (int s = 0; s < 8; s++) {
				assertEquals(0, (int) VmStateLayout.CALL_STACK_ELEMENT_HANDLE.get(state, 0L, (long) s),
						"Call stack slot " + s + " MUST be 0");
			}
		}
	}
}
