...
public class Twitter2010CcAotRunner {
	...
	public static void main(String[] args) throws Throwable {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			// 2: OP_SET_UNION (dst=2, payload=(0 << 16) | 1) -> R2 = R0 U R1 (component
			// set)
			// 3: OP_CSR_WALK (dst=3, payload=(1 << 16) | 0) -> R1 -> R3 (frontier 2)
			// 4: OP_SET_UNION (dst=2, payload=(2 << 16) | 3) -> R2 = R2 U R3 (component
			// set)
			...
			if (warmupCount > 0) {
				...
				System.out.printf("Post-Warmup Latency:               %.4f ms (%.2f µs)%n", hotMhTimeMs,
						hotMhTimeMs * 1000.0);
			}
		}
