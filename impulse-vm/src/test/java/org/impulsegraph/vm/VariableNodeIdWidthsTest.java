...
public class VariableNodeIdWidthsTest {
	...
	private void runMatrixTest(byte srcWidth, byte dstWidth, byte edgeIdxWidth) {
		System.out
				.println("Java Testing combination: Src NodeID Width = " + (srcWidth * 8) + " bit, Dst NodeID Width = "
						+ (dstWidth * 8) + " bit, Edge Index Width = " + (edgeIdxWidth * 8) + " bit...");
		...
		try (Arena arena = Arena.ofConfined()) {
			...
			RelationSnapshot rel = new RelationSnapshot(arena, 4, 6, csrOffsetsSeg, csrTargetsSeg, cscOffsetsSeg,
					cscTargetsSeg, java.util.Collections.<MemorySegment>emptyList(),
					java.util.Collections.<MemorySegment>emptyList(), srcWidth, dstWidth, edgeIdxWidth);
			...
			// Test 1: OP_CSR_DEGREE for node 0 (expected: 2) and node 3 (expected: 0)
			{
				...
				try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
					...
					VmHandlers.handleCsrDegree(state, ctx,
							new VmHandlers.Instruction(OP_CSR_DEGREE, (byte) 0, (short) 0, (0 << 16) | 1));
					assertEquals(2, VmHandlers.getRegisterValue(state, 0));

					VmHandlers.setRegister(state, 2, 3, TYPE_INT64);
					VmHandlers.handleCsrDegree(state, ctx,
							new VmHandlers.Instruction(OP_CSR_DEGREE, (byte) 0, (short) 3, (0 << 16) | 2));
