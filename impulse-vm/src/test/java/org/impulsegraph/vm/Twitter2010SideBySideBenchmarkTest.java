...
/**
 * Leak-free and Thread-Safe High-Precision Side-by-Side Benchmark including
 * Cold-Start & Warmup Passes. Evaluates identical query semantics (full
...
public class Twitter2010SideBySideBenchmarkTest {
	...
	public void runSideBySideBenchmarkWithWarmupPasses() throws Throwable {
		...
		System.out.println(
				"\n=========================================================================================================================");
		System.out.println(
				"   HIGH-PRECISION SIDE-BY-SIDE BENCHMARK INCLUDING COLD-START & WARMUP PASSES (THREAD-SAFE)                             ");
		System.out.println(
				"   Dataset: Twitter-2010 (41.6M Nodes, 1.46B Edges)                                                                      ");
		System.out.println(
				"=========================================================================================================================");
		...
		try (Arena sharedArena = Arena.ofShared()) {
			BinarySnapshotLoader.LoadedSnapshot loadedSnapshot = BinarySnapshotLoader
					.loadSnapshot(TWITTER_SNAPSHOT_PATH, sharedArena);
			...
			System.out.printf("Graph Topology:                                %,d nodes, %,d edges%n%n", nodeCount,
					edgeCount);

			int[][] workloads = new int[][]{{1000000, 999880, 1}, // 1 Hop
			...
			for (int i = 0; i < workloads.length; i++) {
				...
				System.out.println(
						"-------------------------------------------------------------------------------------------------------------------------");
				System.out.printf("%-18s | %-20s | %-20s | %-20s | %-12s%n", "Execution Phase", "Manual Loop (us)",
						"VM Interp (us)", "VM JIT (us)", "Speedup");
				System.out.println(
						"-------------------------------------------------------------------------------------------------------------------------");
				...
				System.out.printf("%-18s | %-20.2f | %-20.2f | %-20.2f | %-12.2fx%n", "Cold-Start (Pass 1)",
						manualColdUs, interpColdUs, jitColdUs, manualColdUs / Math.max(jitColdUs, 0.001));

				// Phase 2: Warmup Passes
				for (int pass = 1; pass <= WARMUP_PASSES; pass++) {
					long t0Mw = System.nanoTime();
					for (int b = 0; b < WARMUP_BATCH_SIZE; b++)
						runManualJavaLoop(rel, src, dst, hops);
					...
					System.out.printf("%-18s | %-20.2f | %-20.2f | %-20.2f | %-12.2fx%n", "Warmup Pass " + pass,
							manualWarmUs, interpWarmUs, jitWarmUs, manualWarmUs / Math.max(jitWarmUs, 0.001));
				}
				...
				System.out.printf("%-18s | %-20.2f | %-20.2f | %-20.2f | %-12.2fx%n",
						"Steady State (N=" + BENCHMARK_ITERATIONS + ")", manualSteadyUs, interpSteadyUs, jitSteadyUs,
						manualSteadyUs / Math.max(jitSteadyUs, 0.001));
				System.out.println();
			}
	...
	private static void setInstruction(MemorySegment prog, int index, byte opcode, byte flags, short dstReg,
			int payload) {
