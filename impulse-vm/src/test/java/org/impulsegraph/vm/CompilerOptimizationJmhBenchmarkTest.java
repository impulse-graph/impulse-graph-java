 ...
 * Measures method-level execution latencies for: 1. O0 (Unoptimized Pipeline:
 * handleCsrWalk + handleSetDifference) 2. O2/O3 (JIT Opcode Fused Micro-Kernel)
 * 3. Monomorphic Handlers (handleCsrWalk, handleSetDifference,
 * handleSetIntersect)
 */
public class CompilerOptimizationJmhBenchmarkTest {
	...
	public void benchmarkMethodLevelJmhStats() throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			RelationSnapshot rel = new org.impulsegraph.storage.csr.RelationSnapshot(arena, nodeCount, edgeCount,
					rowOffsets, colTargets);
			...
			for (int i = 0; i < nodeCount; i += 10)
				revokedSet.set(i);

			VmHandlers.Instruction csrWalkInstr = new VmHandlers.Instruction(VmRegisterType.OP_CSR_WALK, (byte) 0, 4,
					2 | (0 << 16));
			VmHandlers.Instruction setDiffInstr = new VmHandlers.Instruction(VmRegisterType.OP_SET_DIFFERENCE, (byte) 0,
					4, 3);
			VmHandlers.Instruction setIntersectInstr = new VmHandlers.Instruction(VmRegisterType.OP_SET_INTERSECT,
					(byte) 0, 4, 2);
			...
			System.out.println(
					"| Optimization Level / Method Name                     | Method Type / Inlining Profile | Mean Latency (us) | Mean Latency (ns) |   Throughput QPS | Speedup |");
			System.out.println(
					"| :--------------------------------------------------- | :----------------------------- | ----------------: | ----------------: | ---------------: | ------: |");
			System.out.printf("| %-52s | %-30s | %14.2f us | %14.0f ns | %13.0f QPS | %7.2fx |\n",
					"O0 (Unoptimized 2-Pass Pipeline: Walk + Diff)", "Unoptimized Multi-Dispatch", avgO0Us,
					avgO0Us * 1000.0, 1000000.0 / avgO0Us, 1.0);
			System.out.printf("| %-52s | %-30s | %14.2f us | %14.0f ns | %13.0f QPS | %7.2fx |\n",
					"VmHandlers.handleCsrWalk(MemorySegment, VmContext)", "Monomorphic (@ForceInline)", avgM1Us,
					avgM1Us * 1000.0, 1000000.0 / avgM1Us, avgO0Us / avgM1Us);
			System.out.printf("| %-52s | %-30s | %14.2f us | %14.0f ns | %13.0f QPS | %7.2fx |\n",
					"VmHandlers.handleSetDifference(MemorySegment, VmContext)", "Monomorphic (@ForceInline)", avgM2Us,
					avgM2Us * 1000.0, 1000000.0 / avgM2Us, avgO0Us / avgM2Us);
			System.out.println("=========================================================================\n");
		}
	}
}
