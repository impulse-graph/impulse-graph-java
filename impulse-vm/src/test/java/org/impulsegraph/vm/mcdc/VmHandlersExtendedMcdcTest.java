...
/**
 * Extended MC/DC condition independence test suite for VmHandlers and VM
 * Control Flow.
 */
	...
	public void testControlFlowJumpsAndCallStack() {
		try (Arena arena = Arena.ofShared()) {
			...
			try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
				...
				MemorySegment jmpProg = buildProgram(arena, makeInstr(OP_LOAD_CONST_INT, (byte) 0, 1, 10),
				...
				MemorySegment jzProg = buildProgram(arena, makeInstr(OP_LOAD_CONST_INT, (byte) 0, 1, 1),
						makeInstr(OP_JZ, (byte) 0, 0, 3), makeInstr(OP_HALT, (byte) 0, 0, 0));
				ImpulseVmInterpreter.execute(jzProg, 3, graph, 0, arena);

				// JNZ jump vs fallthrough
				MemorySegment jnzProg = buildProgram(arena, makeInstr(OP_LOAD_CONST_INT, (byte) 0, 1, 1),
						makeInstr(OP_JNZ, (byte) 0, 0, 3), makeInstr(OP_HALT, (byte) 0, 0, 0));
				ImpulseVmInterpreter.execute(jnzProg, 3, graph, 0, arena);

				// LOOP_DECR loop
				MemorySegment loopProg = buildProgram(arena, makeInstr(OP_LOAD_CONST_INT, (byte) 0, 10, 3),
