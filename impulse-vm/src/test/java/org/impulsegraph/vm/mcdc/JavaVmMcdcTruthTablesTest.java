...
/**
 * Comprehensive Java ImpulseVM MC/DC Truth Table and Condition Independence
 * Suite. Exhaustively tests all 117 ISA opcode handlers, register boundary
 * conditions, type matrices, bitset operations, and traversal pipelines.
 */
public class JavaVmMcdcTruthTablesTest {
	...
	public void testStateLayoutAndRegisterBoundaries() {
		try (Arena arena = Arena.ofConfined()) {
			...
			byte[] types = {TYPE_NULL, TYPE_INT64, TYPE_FLOAT, TYPE_DOUBLE, TYPE_NODE_ID, TYPE_BITSET_HANDLE,
					TYPE_FLOAT_VECTOR, TYPE_DOUBLE_VECTOR, TYPE_NODE_VECTOR, TYPE_STRING_VECTOR};
			for (byte t : types) {
	...
	public void testInstructionDecodingAndBounds() {
		try (Arena arena = Arena.ofConfined()) {
			MemorySegment prog = buildProgram(arena, makeInstr(OP_NOP, (byte) 0, 0, 0),
					makeInstr(OP_LOAD_CONST_INT, (byte) 0, 1, 42), makeInstr(OP_HALT, (byte) 0, 0, 0));
			...
			// Word 1: [opcode=OP_CSR_WALK_PREDICATE (0x13), flags=0x80 (EXTENDED), dst=2,
			// arg1=0, arg2=1]
			...
			// Word 2: [marker=0xFF, pad=0x00, arg3=40000 (uint16 relation ID), arg4=0,
			// arg5=0]
	...
	public void testVectorMathAndLogicHandlers() {
		try (Arena arena = Arena.ofShared()) {
			...
			try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
				...
				VmHandlers.handleLoadConstFloat(state,
						makeInstr(OP_LOAD_CONST_FLOAT, (byte) 0, 2, Float.floatToRawIntBits(3.14f)));
				assertEquals(TYPE_FLOAT, VmHandlers.getRegisterType(state, 2));

				VmHandlers.handleLoadConstStrPrefix(state,
						makeInstr(OP_LOAD_CONST_STR_PREFIX, (byte) 0, 3, 0x41424344));
				...
				VmHandlers.handleVecCmpGt(state, ctx,
						makeInstr(OP_VEC_CMP_GT, (byte) 0, 11, (10 << 16) | Float.floatToRawIntBits(4.0f)));
				VmHandlers.handleVecCmpLt(state, ctx,
						makeInstr(OP_VEC_CMP_LT, (byte) 0, 12, (10 << 16) | Float.floatToRawIntBits(8.0f)));
				VmHandlers.handleVecCmpEq(state, ctx,
						makeInstr(OP_VEC_CMP_EQ, (byte) 0, 13, (10 << 16) | Float.floatToRawIntBits(5.0f)));
				...
				// 6. Vector Math Unary (15 functions), Binary (11 functions), Ternary (4
				// functions)
				...
				for (int funcId = 0; funcId <= 10; funcId++) {
					VmHandlers.handleVecMathBinary(state, ctx,
							makeInstr(OP_VEC_MATH_BINARY, (byte) funcId, 21, (10 << 16) | 10));
				}
				for (int funcId = 0; funcId <= 3; funcId++) {
					VmHandlers.handleVecMathTernary(state, ctx,
							makeInstr(OP_VEC_MATH_TERNARY, (byte) funcId, 22, (10 << 16) | 10));
				}
				...
				Object argMaxRes = VmHandlers.handleVectorReduceArgMax(state, ctx,
						makeInstr(OP_REDUCE, (byte) 0, 10, 0));
				assertNotNull(argMaxRes);

				Object argMinRes = VmHandlers.handleVectorReduceArgMin(state, ctx,
						makeInstr(OP_REDUCE, (byte) 0, 10, 0));
				assertNotNull(argMinRes);

				for (int opId = 0; opId <= 5; opId++) {
					Object redDispatch = VmHandlers.handleReduce(state, ctx,
							makeInstr(OP_REDUCE, (byte) 0, 10, (opId << 16)));
	...
	public void testGraphWalkHandlersAndWidths() {
		try (Arena arena = Arena.ofShared()) {
			...
			try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
				...
				// Seed input set {0, 1}
				VmHandlers.handleInitInputSet(state, ctx, makeInstr(OP_INIT_INPUT_SET, (byte) 0, 1, 0),
						new int[]{0, 1});
	...
	public void testExtendedDomainAlgorithms() {
		try (Arena arena = Arena.ofShared()) {
			...
			try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
				...
				// Roaring Bitmaps ops
				VmHandlers.handleRoaringBitmapAnd(state, ctx,
						makeInstr(OP_ROARING_BITMAP_AND, (byte) 0, 14, (12 << 16) | 13));
				VmHandlers.handleRoaringBitmapOr(state, ctx,
						makeInstr(OP_ROARING_BITMAP_OR, (byte) 0, 15, (12 << 16) | 13));
				VmHandlers.handleRoaringBitmapAndNot(state, ctx,
						makeInstr(OP_ROARING_BITMAP_AND_NOT, (byte) 0, 16, (12 << 16) | 13));
	...
	public void testTraversalDslAndStatementApi() {
		try (Arena arena = Arena.ofShared()) {
			...
			DefaultTraversal<ImpulseBitSet> travProject = (DefaultTraversal<ImpulseBitSet>) travFilter
					.project("score = 100.0");
	...
	public void testCompiledQueryAndCompilerPasses() {
		try (Arena arena = Arena.ofShared()) {
			MemorySegment prog = buildProgram(arena, makeInstr(OP_INIT_INPUT_NODE, (byte) 0, 0, 0),
					makeInstr(OP_CSR_WALK, (byte) 0, 1, 0), makeInstr(OP_VECTOR_REDUCE_SUM, (byte) 0, 1, 0),
					makeInstr(OP_COLLECT_BITSET, (byte) 0, 1, 0), makeInstr(OP_HALT, (byte) 0, 0, 0));
			...
			VmHandlers.Instruction[] instrs = new VmHandlers.Instruction[]{
					makeInstr(OP_INIT_INPUT_NODE, (byte) 0, 0, 0), makeInstr(OP_CSR_WALK, (byte) 0, 1, 0),
