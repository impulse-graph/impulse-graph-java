...
/**
 * MC/DC condition independence tests for specialized graph structure
 * introspection, vector division, string concatenation, scratch memory
 * indexing, and assertion opcodes.
 */
	...
	public void testVectorDivAndStringOperations() {
		try (Arena arena = Arena.ofShared()) {
			...
			try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
				...
				System.out.println(
						"hNum=" + hNum + " hDen=" + hDen + " dstHandle=" + VmHandlers.getRegisterValue(state, 3));
	...
	public void testInlineArraysAndAssertions() {
		try (Arena arena = Arena.ofShared()) {
			...
			try (VmQueryContext ctx = new VmQueryContext(graph, arena)) {
				...
				assertThrows(IllegalStateException.class, () -> VmHandlers.handleLoadInlineArray(state, ctx,
						makeInstr(OP_LOAD_INLINE_ARRAY, (byte) 0, 1, 0)));
				...
				VmHandlers.handleLoadInlineArray(state, ctx,
						makeInstr(OP_LOAD_INLINE_ARRAY, (byte) 0, 1, (2 << 16) | 0));
