...
/**
 * Test Suite verifying Floating Point Execution, NaN/Inf traps,
 * OP_ASSERT_FINITE, and vector math error handling in the Java ImpulseVM
 * engine.
 */
public class FloatingPointExecutionTest {
	...
	void testAssertFiniteScalarRegisters() {
		try (Arena arena = Arena.ofConfined(); VmQueryContext ctx = new VmQueryContext(null, arena)) {
			...
			assertDoesNotThrow(() -> VmHandlers.handleAssertFinite(state, ctx,
					new VmHandlers.Instruction(OP_ASSERT_FINITE, (byte) 0, 10, 0)));

			// NaN scalar float -> Throws
			VmHandlers.setRegister(state, 11, Float.floatToRawIntBits(Float.NaN), TYPE_FLOAT);
			IllegalStateException exNan = assertThrows(IllegalStateException.class, () -> VmHandlers
					.handleAssertFinite(state, ctx, new VmHandlers.Instruction(OP_ASSERT_FINITE, (byte) 0, 11, 0)));
			...
			IllegalStateException exInf = assertThrows(IllegalStateException.class, () -> VmHandlers
					.handleAssertFinite(state, ctx, new VmHandlers.Instruction(OP_ASSERT_FINITE, (byte) 0, 12, 0)));
