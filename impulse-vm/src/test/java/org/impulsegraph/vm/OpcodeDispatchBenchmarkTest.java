...
/**
 * High-Resolution Opcode Dispatch Stress-Test Benchmark for Java 25 ImpulseVM
 * Interpreter.
 */
public class OpcodeDispatchBenchmarkTest {
	...
	public void runAllDispatchBenchmarks() {
		...
		System.out.println();
		System.out.printf("%-52s %-14s %-12s %-12s %-12s%n", "Benchmark Scenario", "Dispatches", "Latency(ms)", "MDOPS",
				"ns/Opcode");
		System.out.println("-".repeat(105));

		for (BenchmarkResult res : results) {
			System.out.printf("%-52s %-14d %-12.2f %-12.1f %-12.2f%n", res.name, res.totalDispatches, res.avgLatencyMs,
					res.mdops, res.nsPerOp);
		}
		System.out.println("-".repeat(105));
	}
	...
	private BenchmarkResult benchBranchPredict(int iterations) {
		try (Arena arena = Arena.ofShared()) {
			...
			return new BenchmarkResult("Branch Predictor & Jumps (OP_JMP / OP_LOOP_DECR)", totalOps, avgMs, mdops,
					nsPerOp);
		}
	}
	...
	private BenchmarkResult benchSubroutineTrampoline(int iterations) {
		try (Arena arena = Arena.ofShared()) {
			...
			return new BenchmarkResult("Subroutine Call Trampoline (OP_CALL / OP_RET)", totalOps, avgMs, mdops,
					nsPerOp);
		}
	}
