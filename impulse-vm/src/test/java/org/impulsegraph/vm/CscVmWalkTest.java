...
public class CscVmWalkTest {
    ...
    void testCscWalkThrowsIfMissing() {
        try (Arena arena = Arena.ofShared()) {
            ...
            RelationSnapshot rel = new org.impulsegraph.storage.csr.RelationSnapshot(arena, nodeCount, 4, offsets, targets);
    ...
    void testSnapshotBuilderGeneratesCscAndWalkSucceeds() throws Exception {
        try (Arena arena = Arena.ofShared()) {
            ...
            RelationSnapshot origRel = new org.impulsegraph.storage.csr.RelationSnapshot(arena, nodeCount, 4, offsets, targets);
            ImpulseGraphSnapshot origGraph = new GraphSnapshot(arena, Map.of("rel_0", origRel));
            ...
            byte[] snapshotBytes = new DefaultSnapshotBuilder()
                    ...
                    .build(new BinarySnapshotLoader.DefaultLoadedSnapshot(
                            (short) 0x494D, (short) 0x0009, (org.impulsegraph.storage.csr.GraphSnapshot) origGraph, Map.of(), Map.of(), Map.of(), Map.of()
                    ));
            ...
            ImpulseGraphSnapshot loadedGraph = (org.impulsegraph.storage.csr.GraphSnapshot) loaded.graph();
            RelationSnapshot loadedRel = (org.impulsegraph.storage.csr.RelationSnapshot) loadedGraph.getRelationSnapshot("rel_0");
