...
public class CscVmWalkTest {
	...
	void testCscWalkThrowsIfMissing() {
		try (Arena arena = Arena.ofShared()) {
			...
			// 1: OP_CSC_WALK (dst=1, payload=(0 << 16) | 0) -> R0 node 1 -> R1 bitset of
			// in-neighbors
	...
	void testSnapshotBuilderGeneratesCscAndWalkSucceeds() throws Exception {
		try (Arena arena = Arena.ofShared()) {
			...
			byte[] snapshotBytes = new DefaultSnapshotBuilder().withCsc(true)
					.build(new BinarySnapshotLoader.DefaultLoadedSnapshot((short) 0x494D, (short) 0x0009,
							(org.impulsegraph.storage.csr.GraphSnapshot) origGraph, Map.of(), Map.of(), Map.of(),
							Map.of()));
			...
			RelationSnapshot loadedRel = (org.impulsegraph.storage.csr.RelationSnapshot) loadedGraph
					.getRelationSnapshot("rel_0");
