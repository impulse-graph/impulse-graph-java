...
public class JavaVmJsonRunner {
    public static void main(String[] args) throws Exception {
        ...
        java.lang.reflect.Method parseMethod = JavaVmPolyglotAssemblyVerifierTest.class.getDeclaredMethod("parseImpasFile", Path.class);
        parseMethod.setAccessible(true);
        JavaVmPolyglotAssemblyVerifierTest.ParsedAssembly asm = (JavaVmPolyglotAssemblyVerifierTest.ParsedAssembly) parseMethod.invoke(verifier, impasFile);
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            try (VmQueryContext ctx = new VmQueryContext(null, arena)) {
                ...
                loop: while (actualStatus.equals("IMPULSE_VM_OK") && pc >= 0 && pc < instructionCount && stepCount++ < 2000000000) {
                    if (fuel > 0) {
                        fuel--;
                        if (fuel == 0) {
                    ...
                    if (instr.dstReg() >= 64 && Byte.toUnsignedInt(opcode) != 0x09 && Byte.toUnsignedInt(opcode) != 0x0B && Byte.toUnsignedInt(opcode) != 0x0C) {
                        System.err.println("INVALID REG in " + impasFile.getFileName() + " at pc=" + pc + ": opcode=0x" + Integer.toHexString(opcode & 0xFF) + ", dstReg=" + instr.dstReg());
                    ...
                    try {
                        switch (Byte.toUnsignedInt(opcode)) {
                            ...
                            case 0x03 -> { VmHandlers.handleInitInputSet(state, ctx, instr, new org.impulsegraph.api.bitset.OffHeapBitSet(arena, 1000)); pc++; }
                            ...
                            case 0x0A, 0x4C, 0x4D, 0x4E -> { VmHandlers.handleLoadInlineSet(state, ctx, instr); pc++; }
                            case 0x0B, 0x0C -> {
                            ...
                            case 0x0D -> { VmHandlers.handleLoadInlineIntArray(state, ctx, instr); pc++; }
                            ...
                            case 0x11 -> { VmHandlers.handleCsrWalkFiltered(state, ctx, instr); pc++; }
                            ...
                            case 0x17 -> { VmHandlers.handleCsrWalkReduce(state, ctx, instr); pc++; } // OP_CSR_WALK_REDUCE
                            ...
                            case 0x33 -> { 
                                ...
                                if (impasFile.getFileName().toString().contains("tc_poly_node_filter")) {
                                    System.err.println("SET_CARD in " + impasFile.getFileName() + " R" + instr.dstReg() + "=" + VmHandlers.getRegisterValue(state, instr.dstReg()) + " (from R" + (instr.payload() & 0xFFFF) + ")");
                                }
                                pc++; 
                            }
                            ...
                            case 0x40 -> { VmHandlers.handleCcAfforest(state, ctx, instr); pc++; }
                            case 0x41 -> { VmHandlers.handleMxv(state, ctx, instr); pc++; }
                            case 0x42 -> { VmHandlers.handleVxm(state, ctx, instr); pc++; }
                            case 0x43 -> { VmHandlers.handleEwiseAdd(state, ctx, instr); pc++; }
                            case 0x44 -> { VmHandlers.handleEwiseMult(state, ctx, instr); pc++; }
                            case 0x45 -> { VmHandlers.handleReduce(state, ctx, instr); pc++; }
                            case 0x46 -> { VmHandlers.handleCcHookCompress(state, ctx, instr); pc++; }
                            case 0x47 -> { VmHandlers.handleTcSweepBatch(state, ctx, instr); pc++; }
                            case 0x48 -> { VmHandlers.handleBrandesForward(state, ctx, instr); pc++; }
                            case 0x49 -> { VmHandlers.handleBrandesBackward(state, ctx, instr); pc++; }
                            case 0x4A -> { VmHandlers.handleDeltaStepRelax(state, ctx, instr); pc++; }
                            ...
                            case 0x5A -> { VmHandlers.handleThrow(state, instr); actualStatus = "IMPULSE_VM_ERR_USER_THROW"; break loop; }
                            case 0x5B -> { VmHandlers.handleAssert(state, instr); pc++; }
                            case 0x1D -> { VmHandlers.handleCsrWalk(state, ctx, instr); pc++; } // OP_ADAPTIVE_WALK
                            case 0x1E -> { VmHandlers.handleCreateScratchIndex(state, instr, ctx); pc++; } // OP_CREATE_SCRATCH_INDEX
                            case 0x1F -> { pc++; } // OP_DROP_SCRATCH_INDEX
                            case 0x20 -> { VmHandlers.handleVecCmpEq(state, ctx, instr); pc++; }
                            case 0x21 -> { VmHandlers.handleVecCmpGt(state, ctx, instr); pc++; }
                            case 0x22 -> { VmHandlers.handleVecCmpLt(state, ctx, instr); pc++; }
                            case 0x23 -> { VmHandlers.handleVecCmpBetween(state, ctx, instr); pc++; }
                            case 0x24 -> { VmHandlers.handleMaskAnd(state, ctx, instr); pc++; }
                            case 0x25 -> { VmHandlers.handleMaskOr(state, ctx, instr); pc++; }
                            case 0x26 -> { VmHandlers.handleMaskNot(state, ctx, instr); pc++; }
                            case 0x27 -> { VmHandlers.handleVecBlend(state, ctx, instr); pc++; }
                            ...
                            case 0x60, 0x61, 0x62, 0x63, 0x66, 0x67, 0x69, 0x6A -> { VmHandlers.validateReg(instr.dstReg()); pc++; }
                            case 0x68 -> { VmHandlers.handleKcoreDecomposition(state, ctx, instr); pc++; }
                            ...
                            case 0x6C -> { VmHandlers.handleRoaringBitmapAndNot(state, ctx, instr); pc++; }
                            case 0x65 -> { VmHandlers.handleIslandDetect(state, ctx, instr); pc++; }
                            case 0x70 -> { VmHandlers.handleMov(state, ctx, instr); pc++; }
                            ...
                            case 0x80 -> { VmHandlers.handleLoadColumnVector(state, ctx, instr); pc++; }
                            case 0x81 -> { VmHandlers.handleGatherNodeAttr(state, ctx, instr); pc++; }
                            case 0x82 -> { VmHandlers.handleGatherEdgeAttr(state, ctx, instr); pc++; }
                            case 0x83 -> { VmHandlers.handleBrinZoneSkip(state, ctx, instr); pc++; }
                            case 0x84 -> { VmHandlers.handleCsrWalkDirectStore(state, ctx, instr); pc++; }
                            case 0x85 -> { VmHandlers.handleCsrWalkDenseStream(state, ctx, instr); pc++; }
                            case 0x86 -> { VmHandlers.handleCooWalk(state, ctx, instr); pc++; }
                            case 0x87 -> { VmHandlers.handleCscWalkDirectStore(state, ctx, instr); pc++; }
                            case 0x88 -> { VmHandlers.handleFixpointKleeneStar(state, ctx, instr); pc++; }
                            case 0x89 -> { VmHandlers.handleSwapReg(state, instr); pc++; }
                            case 0x8A -> { VmHandlers.handleFrontierDiff(state, ctx, instr); pc++; }
                            case 0x8B -> { VmHandlers.handleCooWalkFiltered(state, ctx, instr); pc++; }
                            case 0x8C -> { VmHandlers.handleCooWalkReduce(state, ctx, instr); pc++; }
                            case 0x8D -> { VmHandlers.handleCooWalkDirectStore(state, ctx, instr); pc++; }
                            case 0x8E -> { VmHandlers.handleDenseWalk(state, ctx, instr); pc++; }
                            case 0x8F -> { VmHandlers.handleDenseWalkBitmatrix(state, ctx, instr); pc++; }
                            case 0x90 -> { VmHandlers.handleCollectBitset(state, ctx, instr); pc++; }
                            case 0x91 -> { VmHandlers.handleCollectArray(state, ctx, instr); pc++; }
                            case 0x92 -> { VmHandlers.handleMapDenseToKeys(state, ctx, instr); pc++; }
                            ...
                            default -> {
                                System.err.println("INVALID OPCODE in " + impasFile.getFileName() + " at pc=" + pc + ": 0x" + Integer.toHexString(opcode & 0xFF));
                                actualStatus = "IMPULSE_VM_ERR_INVALID_OPCODE";
                                break loop;
                            }
                        }
                    } catch (Throwable t) {
                        System.err.println("Crash in " + impasFile.getFileName() + " at pc=" + pc + " (op 0x" + Integer.toHexString(opcode & 0xFF) + "): " + t);
