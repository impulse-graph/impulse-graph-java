...
/**
 * Impulse VM Bytecode Interpreter. Executes native bytecodes lock-free and
 * zero-allocation over off-heap memory segments.
 */
public final class ImpulseVmInterpreter {
	...
	public static Object execute(MemorySegment programSeg, long instructionCount, ImpulseGraphSnapshot snapshot,
			Object input, Arena arena) {
		return execute(programSeg, instructionCount, snapshot, input, arena, null);
	}

	public static Object execute(MemorySegment programSeg, long instructionCount, ImpulseGraphSnapshot snapshot,
			Object input, Arena arena, java.util.List<String> stringPool) {
				...
				switch (instr.opcode()) {
					...
					case OP_CSR_WALK -> {
						...
						if ((instr.flags() & VmHandlers.FLAG_HALT_ON_EMPTY) != 0
								&& VmHandlers.checkFlag(state, FLAG_ZF)) {
					...
					case OP_STREAM_FUNC_BEGIN, OP_STREAM_FUNC_END, OP_STREAM_LOAD_SRC, OP_STREAM_LOAD_EDGE,
							OP_STREAM_LOAD_TGT, OP_STREAM_LOAD_SRC_ID, OP_STREAM_LOAD_TGT_ID, OP_STREAM_LOAD_EDGE_ID,
							OP_STREAM_LOAD_CONST, OP_STREAM_MATH_ADD, OP_STREAM_MATH_SUB, OP_STREAM_MATH_MUL,
							...
							OP_STREAM_SCATTER_REDUCE, OP_STREAM_FILTER -> {
						pc++;
					}
					...
					case OP_VECTOR_LOAD_ATTR -> {
						if (DEBUG_MODE)
							LOG.info("BEFORE handleVectorLoadAttr pc=" + pc);
						try {
							VmHandlers.handleVectorLoadAttr(state, ctx, instr);
						} catch (Throwable t) {
							if (DEBUG_MODE)
								LOG.log(Level.SEVERE, "Exception inside OP_VECTOR_LOAD_ATTR!", t);
							throw t;
						}
						if (DEBUG_MODE)
							LOG.info("AFTER handleVectorLoadAttr pc=" + pc);
						pc++;
					}
					...
					case OP_RESERVED_0A, OP_RESERVED_0B, OP_RESERVED_0C, OP_RESERVED_0D, OP_RESERVED_28, OP_RESERVED_29,
							OP_RESERVED_2B, OP_RESERVED_2C, OP_RESERVED_3E, OP_RESERVED_3F, OP_RESERVED_4C,
							OP_RESERVED_4D, OP_RESERVED_4E, OP_RESERVED_4F, OP_RESERVED_59, OP_RESERVED_5D,
							OP_RESERVED_5E, OP_RESERVED_5F, OP_RESERVED_6D, OP_RESERVED_6E, OP_RESERVED_6F,
							OP_RESERVED_76, OP_RESERVED_77, OP_RESERVED_78, OP_RESERVED_79, OP_RESERVED_7A,
							OP_RESERVED_7B, OP_RESERVED_7C, OP_RESERVED_7D, OP_RESERVED_7E, OP_RESERVED_7F -> {
						throw new IllegalStateException("IMPULSE_VM_ERR_RESERVED_OPCODE");
					}

					default -> throw new IllegalArgumentException(
							"Unknown opcode: 0x" + Integer.toHexString(instr.opcode() & 0xFF));
				}
			}
