...
public class RbacSnapshotQueryExample {
	...
	public static void main(String[] args) throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			System.out.printf("[+] Snapshot Magic: 0x%X | Version: %d | SHA256: %s%n", loadedSnapshot.magic(),
					loadedSnapshot.version(), loadedSnapshot.getSha256Checksum());
			...
			// 3. Define 3-tier User -> Group -> Role RBAC query via fluent AST builder
			// (walkthrough Step 5)
			ImpulseGraphQuery<Object> userToRolesQuery = ImpulseGraphQuery.builder()
					.input("USER", ArgType.ROARING_BITSET).walkEdge("userToGroup").walkEdge("groupToRole")
			...
			if (userToGroup != null && groupToRole != null) {
				...
				System.out.printf("    User %d -> Groups %s (count=%d)%n", startUser, java.util.Arrays.toString(groups),
						groups.length);
