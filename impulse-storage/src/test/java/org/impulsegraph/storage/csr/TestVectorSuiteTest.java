...
class TestVectorSuiteTest {
	...
	public record TestManifest(String name, String description, String spec_version, int domain_count,
			int relation_count, long total_nodes, long total_edges, String sha256, String expected_status) {
	}
	...
	private static Path getSpecTestVectorsDir() {
		...
		if (curr == null) {
			throw new IllegalStateException(
					"Workspace root containing 'impulse-graph-spec' directory not found. Spec testing (.imps) is MANDATORY.");
		}
	...
	static Stream<Path> testVectorDirectoriesProvider() throws IOException {
		...
		try (Stream<Path> stream = Files.list(specDir)) {
			List<Path> dirs = stream.filter(Files::isDirectory)
					...
					.filter(p -> isV09Manifest(p.resolve("manifest.json"))).sorted().toList();
	...
	void testLoadAndVerifyTestVector(Path testVectorDir) throws Exception {
		...
		if (shouldSucceed) {
			try (Arena arena = Arena.ofShared()) {
				...
				assertEquals(manifest.domain_count(), loaded.domainCount(),
						"Domain count mismatch in " + manifest.name());
				assertEquals(manifest.relation_count(), loaded.relationCount(),
						"Relation count mismatch in " + manifest.name());
				if (loaded.getSha256Checksum() != null && !loaded.getSha256Checksum().isEmpty()) {
					assertEquals(manifest.sha256().toLowerCase(), loaded.getSha256Checksum().toLowerCase(),
							"SHA-256 checksum mismatch in " + manifest.name());
				}

				GraphSnapshot graph = loaded.graph();
				assertNotNull(graph, "GraphSnapshot MUST NOT be null");
				assertEquals(manifest.relation_count(), graph.getAllRelationSnapshots().size(),
						"Relation map size mismatch");

				System.out.printf("  [PASS] %-35s -> Domains: %,d | Relations: %,d | Off-heap: %,d bytes%n",
						manifest.name(), loaded.domainCount(), loaded.relationCount(),
						graph.getOffHeapMemorySizeBytes());
			}
		} else {
			// Expected Failure test cases (e.g. SHA256 bit-flip corruption, unsupported
			// global features)
