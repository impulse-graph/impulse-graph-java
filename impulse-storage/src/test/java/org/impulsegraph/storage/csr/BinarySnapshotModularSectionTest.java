...
public class BinarySnapshotModularSectionTest {
	...
	static void setupFixtures() {
		...
		org.junit.jupiter.api.Assumptions.assumeTrue(Files.exists(fixtureUnoptRaw),
				"Fixture livejournal_unopt_raw.bin not found");
		org.junit.jupiter.api.Assumptions.assumeTrue(Files.exists(fixtureUnoptVbyte),
				"Fixture livejournal_unopt_vbyte.bin not found");
		org.junit.jupiter.api.Assumptions.assumeTrue(Files.exists(fixtureOptRaw),
				"Fixture livejournal_opt_raw.bin not found");
		org.junit.jupiter.api.Assumptions.assumeTrue(Files.exists(fixtureOptVbyte),
				"Fixture livejournal_opt_vbyte.bin not found");
		org.junit.jupiter.api.Assumptions.assumeTrue(Files.exists(fixtureOptHybrid),
				"Fixture livejournal_opt_hybrid.bin not found");
	}
	...
	void testEncodingMatrixSupport() throws IOException {
		BinarySnapshotLoader.LoadedSnapshot s1 = BinarySnapshotLoader.loadSnapshot(Files.readAllBytes(fixtureUnoptRaw),
				Arena.ofAuto());
		assertNotNull(s1);
		assertEquals(1000000, s1.graph().getAllRelationSnapshots().values().iterator().next().getEdgeCount());

		BinarySnapshotLoader.LoadedSnapshot s2 = BinarySnapshotLoader
				.loadSnapshot(Files.readAllBytes(fixtureUnoptVbyte), Arena.ofAuto());
		assertNotNull(s2);
		assertEquals(1000000, s2.graph().getAllRelationSnapshots().values().iterator().next().getEdgeCount());

		BinarySnapshotLoader.LoadedSnapshot s3 = BinarySnapshotLoader.loadSnapshot(Files.readAllBytes(fixtureOptRaw),
				Arena.ofAuto());
		assertNotNull(s3);
		assertEquals(1000000, s3.graph().getAllRelationSnapshots().values().iterator().next().getEdgeCount());

		BinarySnapshotLoader.LoadedSnapshot s4 = BinarySnapshotLoader.loadSnapshot(Files.readAllBytes(fixtureOptVbyte),
				Arena.ofAuto());
		assertNotNull(s4);
		assertEquals(1000000, s4.graph().getAllRelationSnapshots().values().iterator().next().getEdgeCount());

		BinarySnapshotLoader.LoadedSnapshot s5 = BinarySnapshotLoader.loadSnapshot(Files.readAllBytes(fixtureOptHybrid),
				Arena.ofAuto());
		assertNotNull(s5);
	...
	void testBusinessKeyMappingIntegrity() throws IOException {
		BinarySnapshotLoader.LoadedSnapshot s = BinarySnapshotLoader.loadSnapshot(Files.readAllBytes(fixtureOptHybrid),
				Arena.ofAuto());
