...
/**
 * Builds and verifies production C-ABI Binary Snapshot files (.imps) for: 1.
 * Hetionet v1.0 (47,031 nodes across 11 entity types, 2,250,197 edges across 24
 * relations) 2. DRKG (97,238 nodes across 13 entity types, 5,874,261 edges
 * across 107 relations) with CSR, CSC, and COO indexes across all relations.
 */
public class HetionetAndDrkgImpsBuilderTest {
	...
	void buildHetionetImps() throws Exception {
		...
		System.out.printf("[*] Loaded %,d Hetionet nodes across %d distinct domains: %s%n", totalNodes,
				distinctKinds.size(), distinctKinds);
			...
			while ((line = br.readLine()) != null) {
				...
				if (parts.length >= 3) {
					...
					if (srcId != null && tgtId != null) {
						relationEdges.computeIfAbsent(relName, k -> new ArrayList<>()).add(new long[]{srcId, tgtId});
					}
				}
			}
		}

		System.out.printf("[*] Ingested %d relation types with total %,d edges%n", relationEdges.size(),
				relationEdges.values().stream().mapToInt(List::size).sum());
			...
			while ((line = br.readLine()) != null) {
				...
				if (parts.length >= 3) {
					...
					if (srcKind != null && tgtKind != null) {
						relSrcTgtDomains.put(relName,
								new int[]{domainNameToId.get(srcKind), domainNameToId.get(tgtKind)});
					}
				}
			}
		}
		...
		try (Arena arena = Arena.ofShared()) {
			...
			DefaultSnapshotBuilder builder = new DefaultSnapshotBuilder().withMetadata("dataset_name", "hetionet-v1.0")
					.withMetadata("source", "https://github.com/hetio/hetionet")
					.withMetadata("node_count", String.valueOf(totalNodes))
					.withMetadata("edge_count",
							String.valueOf(relationEdges.values().stream().mapToInt(List::size).sum()))
					.withCsc(true).withCoo(true);
			...
			byte[] impsBytes = builder.build(new BinarySnapshotLoader.DefaultLoadedSnapshot(0x494D5053, (short) 9,
					graphSnapshot, Map.of(), Map.of(), Map.of(), Map.of()));

			try (FileOutputStream fos = new FileOutputStream(outImpsPath.toFile())) {
				fos.write(impsBytes);
			}

			System.out.printf("[+] Successfully compiled Hetionet to %s (%,d bytes / %.2f MB)%n", outImpsPath,
					impsBytes.length, impsBytes.length / (1024.0 * 1024.0));
	...
	void buildDrkgImps() throws Exception {
		...
		System.out.printf("[*] Loaded %,d DRKG entities across %d domains: %s%n", totalNodes, entityDomains.size(),
				entityDomains);
		...
		System.out.printf("[*] Ingested %d DRKG relations with total %,d edges%n", relationEdges.size(), totalEdges);
			...
			while ((line = br.readLine()) != null) {
				...
				if (parts.length >= 3) {
					...
					if (srcDom != null && tgtDom != null) {
						relSrcTgtDomains.put(relName,
								new int[]{domainNameToId.get(srcDom), domainNameToId.get(tgtDom)});
					}
				}
			}
		}
		...
		try (Arena arena = Arena.ofShared()) {
			...
			GraphSnapshot graphSnapshot = new GraphSnapshot(arena, relSnapshots);
			System.out.println(
					"[*] Compiling DRKG GraphSnapshot to .imps binary with CSR, CSC, COO across 107 relations...");

			DefaultSnapshotBuilder builder = new DefaultSnapshotBuilder().withMetadata("dataset_name", "DRKG")
					.withMetadata("source", "https://github.com/gnn4dr/DRKG")
					.withMetadata("node_count", String.valueOf(totalNodes))
					.withMetadata("edge_count", String.valueOf(totalEdges)).withCsc(true).withCoo(true);
			...
			System.out.printf("[+] Successfully compiled DRKG to %s (%,d bytes / %.2f MB)%n", outImpsPath,
					impsBytes.length, impsBytes.length / (1024.0 * 1024.0));
