...
class ManifestMultiFileTest {
	...
	void testLoadFromManifest() throws IOException {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			// The rust tool generates relations with the name we provided via
			// --relation-name
			org.impulsegraph.storage.csr.RelationSnapshot follows = (org.impulsegraph.storage.csr.RelationSnapshot) graph
					.getRelationSnapshot("Follows");
			...
			org.impulsegraph.storage.csr.RelationSnapshot joinedGroup = (org.impulsegraph.storage.csr.RelationSnapshot) graph
					.getRelationSnapshot("JoinedGroup");
