    ...
    void testReachabilityQueryOnR12345WarmedUp() throws Exception {
        ...
        try (Arena arena = Arena.ofShared()) {
            ...
            for (int w = 0; w < 50_000; w++) {
                ...
                if (dummy == null) throw new IllegalStateException();
            }
            ...
            System.out.println("Total Edges Traversed:    " + (rowOffsets[activeSeedNode + 1] - rowOffsets[activeSeedNode]));
            System.out.println("-------------------------------------------------");
            System.out.println("Cold Directory Lookup:    " + String.format("%.3f", coldLookupNanos / 1000.0) + " us");
            System.out.println("WARMED Directory Lookup:  " + String.format("%.3f", avgWarmLookupNanos) + " ns (" + String.format("%.6f", avgWarmLookupNanos / 1000.0) + " us)");
            System.out.println("-------------------------------------------------");
            System.out.println("Cold Traversal Latency:   " + String.format("%.3f", coldTraversalNanos / 1000.0) + " us");
            System.out.println("WARMED Traversal Latency: " + String.format("%.2f", avgWarmTraversalNanos) + " ns (" + String.format("%.6f", avgWarmTraversalNanos / 1000.0) + " us)");
            System.out.println("Throughput (Warmed):      " + String.format("%.2f", (traversalIterations / (warmTraversalTotalNanos / 1_000_000_000.0)) / 1_000_000.0) + " Million traversals/sec");
            System.out.println("=================================================");
