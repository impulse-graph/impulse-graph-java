...
/**
 * High-performance off-heap calculator for attribute column zone maps, value
 * distributions, and monotonicity.
 */
public class AttributeStatisticsCalculator {
	...
	public static AttributeStatistics calculateInt32(String name, MemorySegment segment, int count) {
		...
		Monotonicity mono;
		if (isConstant)
			mono = Monotonicity.MONO_CONSTANT;
		else if (isStrictInc)
			mono = Monotonicity.MONO_STRICT_INC;
		else if (isWeakInc)
			mono = Monotonicity.MONO_WEAK_INC;
		else if (isStrictDec)
			mono = Monotonicity.MONO_STRICT_DEC;
		else if (isWeakDec)
			mono = Monotonicity.MONO_WEAK_DEC;
		else
			mono = Monotonicity.MONO_NONE;

		return new AttributeStatistics(name, min == Long.MAX_VALUE ? 0 : min, max == Long.MIN_VALUE ? 0 : max,
				min == Long.MAX_VALUE ? 0.0 : (double) min, max == Long.MIN_VALUE ? 0.0 : (double) max, "", "",
				nullCount, distinct.size(), mono, nullCount > 0);
	}
	...
	public static AttributeStatistics calculateFloat64(String name, MemorySegment segment, int count) {
		...
		return new AttributeStatistics(name, (long) (min == Double.MAX_VALUE ? 0 : min),
				(long) (max == -Double.MAX_VALUE ? 0 : max), min == Double.MAX_VALUE ? 0.0 : min,
				max == -Double.MAX_VALUE ? 0.0 : max, "", "", nullCount, distinct.size(), mono, nullCount > 0);
	}
}
