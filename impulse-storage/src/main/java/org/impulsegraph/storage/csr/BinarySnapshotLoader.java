...
public final class BinarySnapshotLoader {
    ...
    public static LoadedSnapshot loadFromManifest(Path manifestPath, Arena arena) throws IOException {
        ...
        if (basePath == null) basePath = Path.of("");
        ...
        for (Map.Entry<String, org.impulsegraph.api.schema.GraphManifest.TablespaceDef> entry : manifest.tablespaces().entrySet()) {
        ...
        for (Map.Entry<String, org.impulsegraph.api.schema.GraphManifest.RelationDef> relEntry : manifest.relations().entrySet()) {
        ...
        return new DefaultLoadedSnapshot(
            SNAPSHOT_MAGIC, (short) 0x0900, unifiedGraph,
            java.util.Map.of(), java.util.Map.of(), java.util.Map.of(), combinedMetadata
        );
    }
