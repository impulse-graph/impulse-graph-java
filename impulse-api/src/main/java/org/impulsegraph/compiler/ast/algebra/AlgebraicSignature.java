...
/**
 * Universal Algebraic Signature attached to AST nodes during type and property
 * inference. Encapsulates interval bounds, morphism classifications, monoidal
 * and lattice axioms.
 */
public record AlgebraicSignature(IntervalBound interval, MorphismClass morphism, int algebraFlags,
		Monotonicity monotonicity, int homomorphismFlags, boolean isPure, boolean isConstantKnown,
		boolean constantBoolVal, long constantIntVal, double constantFloatVal) {

	public enum MorphismClass {
		GENERAL, FUNCTIONAL, // Out-Degree <= 1 (Many-to-One)
	...
	public record IntervalBound(long minInt, long maxInt, double minFloat, double maxFloat, boolean isBounded) {
		public static final IntervalBound UNBOUNDED = new IntervalBound(Long.MIN_VALUE, Long.MAX_VALUE,
	...
	public static AlgebraicSignature defaultGeneral() {
		return new AlgebraicSignature(IntervalBound.UNBOUNDED, MorphismClass.GENERAL, ALG_NONE, Monotonicity.MONO_NONE,
				HOMO_NONE, true, false, false, 0, 0.0);
	}

	public static AlgebraicSignature ofConstantBool(boolean val) {
		return new AlgebraicSignature(IntervalBound.UNBOUNDED, MorphismClass.GENERAL, ALG_NONE,
				Monotonicity.MONO_CONSTANT, HOMO_DISTRIBUTES_OR | HOMO_DISTRIBUTES_AND, true, true, val, val ? 1 : 0,
				val ? 1.0 : 0.0);
	}

	public static AlgebraicSignature ofConstantInt(long val) {
		return new AlgebraicSignature(IntervalBound.ofConstantInt(val), MorphismClass.GENERAL, ALG_NONE,
				Monotonicity.MONO_CONSTANT, HOMO_COMMUTES_WITH_MAX | HOMO_COMMUTES_WITH_MIN, true, true, val != 0, val,
				(double) val);
	}

	public static AlgebraicSignature ofConstantFloat(double val) {
		return new AlgebraicSignature(IntervalBound.ofConstantFloat(val), MorphismClass.GENERAL, ALG_NONE,
				Monotonicity.MONO_CONSTANT, HOMO_COMMUTES_WITH_MAX | HOMO_COMMUTES_WITH_MIN, true, true, val != 0.0,
				(long) val, val);
	}
	...
	public static AlgebraicSignature ofMorphism(Multiplicity multiplicity) {
		...
		return new AlgebraicSignature(IntervalBound.UNBOUNDED, morphismClass, ALG_NONE, Monotonicity.MONO_NONE,
				HOMO_NONE, true, false, false, 0, 0.0);
	}
