...
/**
 * Domain Anchor Context representing a specific entity domain (e.g. User,
 * WineInventory, Cheese).
 *
 * <p>
 * All graph traversals in multi-domain graphs start from an explicit Domain
 * anchor.
 * </p>
 */
public interface DomainView {
	...
	/**
	 * Initializes frontier with `n` random nodes uniformly sampled from this
	 * domain.
	 */
	...
	/**
	 * Look up the dense node ID (0 ... N_d - 1) for an external business key (e.g.
	 * "DB00001", "user_alice").
	 *
	 * @param key
	 *            External business key string
	 ...
	 *
	 * @param denseId
	 *            Dense node ID (0 ... N_d - 1)
	 * @return External key string, or null/fallback if not found
	 */
	String toKey(long denseId);

	/**
	 * Initializes frontier with a single external business key (e.g. "user_alice",
	 * "DB00001").
	 */
