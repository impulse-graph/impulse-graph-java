...
/**
 * Opt-in Prometheus Metrics Exporter operating on pure JDK 25 standard library
 * with zero third-party dependencies. Generates standard Prometheus exposition
 * text format for HTTP /metrics endpoints.
 */
public class PrometheusMetricsExporter implements ImpulseMetricsRegistry {
	...
	public void recordQueryExecution(long durationNanos) {
		...
		if (seconds <= 0.00005)
			bucket50us.increment();
		if (seconds <= 0.00010)
			bucket100us.increment();
		if (seconds <= 0.00050)
			bucket500us.increment();
		if (seconds <= 0.00100)
			bucket1ms.increment();
		if (seconds <= 0.00500)
			bucket5ms.increment();
		if (seconds <= 0.01000)
			bucket10ms.increment();
		bucketInf.increment();
	}
	...
	public String scrapeMetrics() {
		...
		sb.append("impulse_query_execution_seconds_sum ").append(queryDurationTotalNanos.sum() / 1_000_000_000.0)
				.append("\n");
		...
		sb.append(
				"# HELP impulse_memory_offheap_bytes Total off-heap FFM Arena memory allocated across live relation snapshots\n");
