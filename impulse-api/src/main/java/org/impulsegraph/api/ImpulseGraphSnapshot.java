 ...
 * Backed by memory-mapped off-heap files conforming to C-ABI Binary Snapshot
 * Spec v0.9.0.
 */
public interface ImpulseGraphSnapshot extends AutoCloseable {
	...
	/**
	 * Obtains the off-heap {@link MemorySegment} for the given relation's target
	 * array.
	 */
	...
	/**
	 * Returns the SHA-256 checksum hex string calculated over the snapshot
	 * sections.
	 */
	...
	/**
	 * Returns an unmodifiable map of all custom metadata key-value pairs stored in
	 * this snapshot.
	 */
	...
	/**
	 * Checks if all queries executing against this snapshot have drained (active
	 * count == 0).
	 */
	...
	/**
	 * Awaits all in-flight queries executing against this snapshot to drain, up to
	...
	/**
	 * Awaits all in-flight queries to drain and closes the underlying off-heap
	 * graph resources.
	 */
	...
	default org.impulsegraph.api.traversal.DomainView domain(String domainName) {
		...
		if (nodeCount <= 0 && !getAllRelationSnapshots().isEmpty()) {
			...
			if (first != null)
				nodeCount = first.getNodeCount();
		}
		return org.impulsegraph.api.spi.ImpulseEngineRegistry.getProvider().createDomainView(this, domainName, 0,
				nodeCount);
	}

	/**
	 * Traverses from a single scalar seed node ID on domain 0 (or default single
	 * domain).
	 */
	 ...
	 */
	default org.impulsegraph.api.traversal.Traversal<org.impulsegraph.api.bitset.ImpulseBitSet> traverse(
			long... seeds) {
	...
	default org.impulsegraph.api.statement.ImpulseStatement prepare(String query) {
		return org.impulsegraph.api.spi.ImpulseEngineRegistry.getProvider().createStatement(this, query);
	}

	/**
	 * Convenience static method to load a snapshot from a file path using an
	 * automatic GC-managed Arena. WARNING: Intended for scripting and short-lived
	 * tools. Multi-GB snapshots in long-running processes should use explicitly
	 * managed Arenas to prevent virtual memory exhaustion.
	 */
	static ImpulseGraphSnapshot load(java.nio.file.Path path) {
		return org.impulsegraph.api.spi.ImpulseEngineRegistry.getProvider().loadSnapshot(path,
