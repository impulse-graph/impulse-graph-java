...
/**
 * Strongly-typed, serializable representation of a graph query (e.g.
 * reachability, transitive closure, set algebra, neighborhood extraction).
 *
 * <p>
 * Queries are pure, immutable ImpScheme AST values with no reference to any
 * graph instance.
 * </p>
 *
 * @param <R>
 *            Return type of the query result (e.g. RoaringBitmap, long[],
 *            Boolean, Long)
 */
public interface ImpulseGraphQuery<R> {
	...
	/**
	 * Executes this query against a target {@link ImpulseGraphSnapshot} with the
	 * specified input parameters.
	 */
	R execute(ImpulseGraphSnapshot snapshot, Object input);

	/**
	 * Returns the underlying ImpScheme S-Expression AST program representing this
	 * query.
	 */
	ImpScmNode getAst();

	/**
	 * Returns the string name or structural operation description of this query
	 * root node.
	 */
	...
	/**
	 * Returns a human-readable ImpScheme S-expression text representation of the
	 * query AST.
	 */
