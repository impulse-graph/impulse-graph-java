...
/**
 * Code generator for producing strongly-typed QueryBuilder Java classes from a
 * {@link GraphSchema}. Generates compile-time type-checked Java classes with
 * IDE auto-complete.
 */
public class SchemaCodeGenerator {
	...
	/**
	 * Generates a Map of class names -> Java source code contents for all entities
	 * in the schema.
	 */
	...
	private String generateEntityClass(GraphSchema schema, GraphSchema.EntityDef entity, String className) {
		...
		sb.append("        return new ").append(className).append("<Object>(\"").append(entity.name())
				.append("\", argType);\n");
		sb.append("    }\n\n");
		...
		if (entity.attributes() != null) {
			for (Map.Entry<String, String> attr : entity.attributes().entrySet()) {
				...
				sb.append("    public ").append(className).append("<R> ").append(filterMethod).append("(String op, ")
						.append(javaType).append(" val) {\n");
