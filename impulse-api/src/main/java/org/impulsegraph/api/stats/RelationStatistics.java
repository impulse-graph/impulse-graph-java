...
/**
 * Immutable statistics container representing structural and algebraic
 * properties of a single relation snapshot.
 */
public class RelationStatistics {
	...
	public RelationStatistics(int nodeCount, int edgeCount, int uniqueSourceNodes, int maxDegree, double avgDegree,
			double stdDevDegree, int p50Degree, int p90Degree, int p99Degree, double sparsity,
			ImpulseBitSet supernodeBitSet) {
		this(nodeCount, edgeCount, uniqueSourceNodes, maxDegree, avgDegree, stdDevDegree, p50Degree, p90Degree,
				p99Degree, sparsity, supernodeBitSet, Multiplicity.MANY_TO_MANY, maxDegree, avgDegree, false, false,
				false);
	}

	public RelationStatistics(int nodeCount, int edgeCount, int uniqueSourceNodes, int maxDegree, double avgDegree,
			double stdDevDegree, int p50Degree, int p90Degree, int p99Degree, double sparsity,
			ImpulseBitSet supernodeBitSet, Multiplicity multiplicity, int maxInDegree, double avgInDegree,
			boolean isAcyclic, boolean isSymmetric, boolean isTransitive) {
