...
/**
 * Fluent builder for constructing immutable {@link ImpulseGraphQuery} AST
 * pipelines.
 *
 * <p>
 * Constructs canonical ImpScheme (.impscm) AST programs ({@link ScmProgram})
 * containing CSR/CSC walks, SIMD vector filtering, CEL predicate evaluation,
 * compile-time parameter binding, set operations, fixed and convergent loop
 * iterations, and domain reductions.
 * </p>
 *
 * @param <R>
 *            Expected return type of the query pipeline result.
 */
public class ImpulseQueryBuilder<R> {
	...
	/**
	 * Bind a named parameter value (e.g. "@P1", "FRUIT" or "@minVoltage", 0.95) for
	 * compile-time substitution.
	 *
	 * @param name
	 *            Parameter name (e.g. "@P1", "@threshold")
	 * @param value
	 *            Bound parameter value (String, Long, Double, Boolean, etc.)
	 * @return This builder instance for method chaining
	 */
	 ...
	 *
	 * @param params
	 *            Map of parameter name to value bindings
	 * @return This builder instance for method chaining
	 */
	 ...
	 *
	 * @param relationName
	 *            Name of edge relation (e.g. "userToGroup", "Branch")
	 * @return This builder instance for method chaining
	 */
	...
	public ImpulseQueryBuilder<R> walkEdgeWithState(String relationName, String stateProjections) {
		Objects.requireNonNull(relationName, "relationName must not be null");
		steps.add(new ScmList(List.of(new ScmSymbol("walk-edge-state"), new ScmSymbol(relationName),
	...
	public ImpulseQueryBuilder<R> projectState(String projectionExpr) {
		steps.add(new org.impulsegraph.compiler.ast.ScmStreamProject(
				new ScmCelExpr(projectionExpr != null ? projectionExpr : "")));
		return this;
	}
	...
	/**
	 * Add a filtered CSR edge walk step over the relation name matching a specific
	 * label.
	 *
	 * @param relationName
	 *            Name of edge relation
	 * @param filterLabel
	 *            Attribute or edge filter label
	 * @return This builder instance for method chaining
	 */
	 ...
	 *
	 * @param stepFn
	 *            Sub-builder lambda configuring loop body steps
	 * @param count
	 *            Fixed loop iteration count
	 * @return This builder instance for method chaining
	 */
	public ImpulseQueryBuilder<R> repeat(Function<ImpulseQueryBuilder<R>, ImpulseQueryBuilder<R>> stepFn, int count) {
		...
		steps.add(new ScmList(
				List.of(new ScmSymbol("repeat"), ScmLiteral.ofInt(count), new ScmProgram(subBuilder.steps))));
		return this;
	}
	...
	/**
	 * Repeat a sub-query pipeline until candidate set generation converges (reaches
	 * a fixed point).
	 *
	 * @param stepFn
	 *            Sub-builder lambda configuring loop body steps
	...
	public ImpulseQueryBuilder<R> repeatUntilStable(Function<ImpulseQueryBuilder<R>, ImpulseQueryBuilder<R>> stepFn) {
		...
		subBuilder.bindParameters(this.parameters);
		stepFn.apply(subBuilder);
		steps.add(new ScmList(List.of(new ScmSymbol("repeat-until-stable"), new ScmProgram(subBuilder.steps))));
		return this;
	}
	 ...
	 * @return This builder instance for method chaining
	 */
	public ImpulseQueryBuilder<R> walkEdgeFilteredAttribute(String relationName, String attributeName, String op,
			double value) {
		Objects.requireNonNull(relationName, "relationName must not be null");
		Objects.requireNonNull(attributeName, "attributeName must not be null");
		steps.add(ScmWalk.forward(relationName, new ScmList(List.of(new ScmSymbol("filter-attr"),
				new ScmSymbol(attributeName), new ScmSymbol(op), ScmLiteral.ofFloat(value)))));
		return this;
	}

	/**
	 * Filter active candidate node set by comparing a node attribute against a
	 * numeric threshold.
	 *
	 * @param attributeName
	 *            Node attribute name
	...
	public ImpulseQueryBuilder<R> filterNodeAttribute(String attributeName, String op, double value) {
		...
		steps.add(new ScmVectorFilter(new ScmList(List.of(new ScmSymbol("filter-node"), new ScmSymbol(attributeName),
				new ScmSymbol(op), ScmLiteral.ofFloat(value)))));
		return this;
	}

	/**
	 * Project an arithmetic combination of node and edge attributes onto the active
	 * candidate frontier.
	 *
	 * @param nodeAttribute
	 *            Node attribute name
	...
	public ImpulseQueryBuilder<R> projectExpression(String nodeAttribute, String operator, String edgeAttribute) {
		...
		steps.add(new ScmList(List.of(new ScmSymbol("project-expression"), new ScmSymbol(nodeAttribute),
				new ScmSymbol(operator), new ScmSymbol(edgeAttribute))));
		return this;
	}
	 ...
	 *
	 * @param <T>
	 *            Expected scalar result type (Double)
	 * @return Immutable compiled query object
	 */
	@SuppressWarnings("unchecked")
	public <T> ImpulseGraphQuery<T> reduceSum() {
		steps.add(ScmReduce.sum());
		return new DefaultImpulseGraphQuery<>(entityType, inputArgType, new ScmProgram(steps), (Class<T>) Double.class,
				parameters);
	}
	 ...
	 *
	 * @param <T>
	 *            Expected scalar result type (Integer)
	 * @return Immutable compiled query object
	 */
	@SuppressWarnings("unchecked")
	public <T> ImpulseGraphQuery<T> reduceArgMax() {
		steps.add(ScmReduce.argmax());
		return new DefaultImpulseGraphQuery<>(entityType, inputArgType, new ScmProgram(steps), (Class<T>) Integer.class,
				parameters);
	}
	...
	/**
	 * Early termination reducer: returns as soon as the first matching projected
	 * node/attribute value is found.
	 *
	 * @param <T>
	...
	public <T> ImpulseGraphQuery<T> reduceFirst() {
		...
		return new DefaultImpulseGraphQuery<>(entityType, inputArgType, new ScmProgram(steps), (Class<T>) Object.class,
				parameters);
	}

	/**
	 * Access point for domain-specific extended opcodes (Powergrid island
	 * detection, ReBAC, motifs).
	 *
	...
	public static class ExtendedOps<R> {
		...
		public ImpulseQueryBuilder<R> islandDetect(int src1Reg, int src2Reg) {
			builder.steps.add(new ScmList(
					List.of(new ScmSymbol("island-detect"), ScmLiteral.ofInt(src1Reg), ScmLiteral.ofInt(src2Reg))));
			return builder;
		}

		public ImpulseQueryBuilder<R> rebacCheck(String permission) {
			builder.steps.add(new ScmList(List.of(new ScmSymbol("rebac-check"), ScmLiteral.ofStr(permission))));
			return builder;
		}
	...
	/**
	 * Terminal collect step: materialize final result in the requested return type
	 * format.
	 *
	 * @param returnType
	 *            Requested result format (e.g. {@link ReturnType#ROARING_BITSET},
	 *            {@link ReturnType#NODE_ARRAY})
	 * @param <T>
	 *            Expected return type class
	 * @return Immutable compiled query object
	 */
	@SuppressWarnings("unchecked")
	public <T> ImpulseGraphQuery<T> collect(ReturnType returnType) {
		...
		ScmCollect collectNode = (returnType == ReturnType.NODE_ARRAY)
				? ScmCollect.vector()
				: (returnType == ReturnType.COUNT) ? ScmCollect.scalar() : ScmCollect.bitset();
	...
	public ImpulseGraphQuery<org.impulsegraph.api.bitset.ImpulseBitSet> collectRoaringBitset() {
		return (ImpulseGraphQuery<org.impulsegraph.api.bitset.ImpulseBitSet>) (ImpulseGraphQuery<?>) collect(
				ReturnType.ROARING_BITSET);
	}
	...
	/**
	 * Format AST steps pipeline into a human-readable ImpScheme S-expression text
	 * representation.
	 */
	...
	private static class DefaultImpulseGraphQuery<R> implements ImpulseGraphQuery<R> {
		...
		public DefaultImpulseGraphQuery(String entityType, ArgType inputArgType, ScmProgram ast, Class<R> resultType,
		...
		public R execute(ImpulseGraphSnapshot snapshot, Object input) {
			try {
				...
				var method = evalCls.getMethod("evaluate", ImpulseGraphQuery.class, ImpulseGraphSnapshot.class,
						Object.class);
