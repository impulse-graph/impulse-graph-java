...
/**
 * Demonstrates SIMD-accelerated attribute filtering on node and edge attributes
...
public class SimdFilteredQuerySample {
	...
	public static void main(String[] args) throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			// "potency_ic50 < 50.0" is evaluated directly off-heap with AVX-512 / ARM Neon
			// vector instructions
			ImpulseBitSet potentInhibitors = compoundDomain.from(sourceCompoundId)
					.out("INHIBITOR", "edge.potency_ic50 < 50.0").toBitSet();
			...
			ImpulseBitSet downstreamTargets = geneDomain.from(gene1001, gene1002)
					.out("INTERACTS_WITH", "edge.confidence >= 0.85").out("TREATS", "edge.phase >= 3").toBitSet();
