...
/**
 * Demonstrates loading an immutable .imps snapshot and executing fluent graph
 * traversals.
 */
public class BasicTraversalSample {
	...
	public static void main(String[] args) throws Exception {
		...
		try (Arena arena = Arena.ofShared()) {
			...
			System.out.printf("Snapshot loaded: %d relations across %d MB off-heap memory.%n", snap.getRelationCount(),
			...
			ImpulseBitSet friends = userDomain.from(denseId).out("knows").toBitSet();
			...
			ImpulseBitSet fof = userDomain.from(denseId).out("knows").out("knows").toBitSet();
